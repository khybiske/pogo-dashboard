...
async function loadAllData() {
  ...
  const [pu, candy, wants, mirrors, friends, shinies, events, pvpRoster, pvpConfig, pvpMeta, raids, pvpRosterUL, pvpMetaUL] = await Promise.all([
    ...
    safe('PVPRosterUL'), safe('PVPMetaUL')
  ]);
...
function renderPowerUp() {
  ...
  c.innerHTML = rows.map(({r,i}) => {
    ...
    if (shiny === 'TRUE' && shadow === 'TRUE') {
      ...
      cardBg = 'linear-gradient(135deg, rgba(107,70,193,0.12) 0%, rgba(128,90,213,0.04) 50%, white 100%)';
    }

    return `
    <div class="list-item${shiny==='TRUE'?' card-shiny':''}${shadow==='TRUE'?' card-shadow':''}" draggable="true" data-index="${i}"
...
function renderCandy() {
  ...
  for (const r of candidates) {
    ...
    <div class="list-item" draggable="true" data-alloc-pokemon="${pokemon}"
         style="display:grid;grid-template-columns:auto auto 1fr auto;gap:0;align-items:start;
                padding:6px 8px;${rowBg}margin-bottom:3px;">
      <span class="drag-handle" style="margin-right:6px;padding-top:2px;">☰</span>
      ...
      <div style="font-size:12px;font-weight:600;min-width:0;">
        ...
        ${needStr !== '✅' ? `<div style="font-size:10px;color:#718096;font-weight:400;margin-top:2px;">Needs: ${needStr}</div>` : '<div style="font-size:10px;color:#48BB78;margin-top:2px;">✅ Fully covered</div>'}
      </div>
...
function renderShinyGaps() {
  ...
  c.innerHTML = '<div class="shiny-grid">'+sorted.map(({r,idx})=>{
    ...
    return `
    <div class="${cardClass}" data-idx="${idx}" onclick="cycleShinyPriority(${idx})" style="cursor:pointer;" title="Click to cycle: Want → High Priority → Got it!">
