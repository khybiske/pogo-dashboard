...
function renderCandy() {
  ...
  for (const r of candidates) {
    ...
    <div class="list-item" draggable="true" data-alloc-pokemon="${pokemon}"
         style="display:grid;grid-template-columns:auto auto 1fr auto;gap:0;align-items:center;
      ...
      <div style="font-size:12px;font-weight:600;min-width:0;">
        ...
        ${needStr !== '✅' ? `<div style="font-size:10px;color:#718096;font-weight:400;margin-top:1px;">Needs: ${needStr}</div>` : '<div style="font-size:10px;color:#48BB78;">✅ Fully covered</div>'}
      </div>
      <div style="text-align:right;white-space:nowrap;">
        ${needRC > 0 ? `<div style="display:flex;align-items:center;gap:2px;justify-content:flex-end;margin-bottom:3px;">
          ...
          <input type="number" value="${allocRC}" min="0"
            style="width:40px;text-align:center;font-size:11px;border:1px solid #e2e8f0;border-radius:4px;padding:2px;"
            onchange="rcAllocSet('${pokemon}',false,this.value)">${RC}
          <button class="btn-icon" style="font-size:10px;padding:1px 4px;" onclick="rcAllocAdj('${pokemon}',false,10)">+</button>
          <span style="color:#aaa;margin:0 3px;">→</span>
          <span style="font-size:11px;font-weight:700;color:${stillRC>0?'#E53E3E':'#48BB78'};min-width:28px;">${stillRC}${RC}</span>
        </div>` : ''}
        ${needXL > 0 ? `<div style="display:flex;align-items:center;gap:2px;justify-content:flex-end;">
          ...
          <button class="btn-icon" style="font-size:10px;padding:1px 4px;" onclick="rcAllocAdj('${pokemon}',true,10)">+</button>
          <span style="color:#aaa;margin:0 3px;">→</span>
          <span style="font-size:11px;font-weight:700;color:${stillXL>0?'#E53E3E':'#48BB78'};min-width:28px;">${stillXL}${RCX}</span>
        </div>` : ''}
