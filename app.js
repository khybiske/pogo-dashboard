// GENERATED: 2026-02-18-2010-FINAL-REBUILD
// Complete rebuild - all sections and modals verified
...
const MIRROR_HEADERS = ['Pokemon','Types','Shiny','Legacy','Source','Notes'];
...
function renderLuckyWants() {
  ...
  const sorted = [...luckyWantsData].sort((a,b)=>(prioOrder[a[2]]||2)-(prioOrder[b[2]]||2));
  ...
  c.innerHTML = sorted.map((r,idx)=>`
    <div class="list-item" draggable="true" data-index="${idx}">
      ...
      <div class="list-item-actions">
        <button class="btn-icon" onclick="editLuckyWant(${idx})" title="Edit">✏️</button>
        <button class="btn-icon" onclick="deleteLuckyWant(${idx})" title="Delete">🗑️</button>
      </div>
    </div>`).join('');
...
function renderMirrorTargets() {
  ...
  const allMirrors = [...autoSynced, ...manual];
...
function renderLuckyFriends() {
  ...
  c.innerHTML = luckyFriendsData.map((r,idx)=>{
    ...
    <div class="list-item" draggable="true" data-index="${idx}">
      ...
      <div class="list-item-content">
        <div class="list-item-line1">
          ${r[0]} ${statusIcon} ${isReady ? 'Ready' : 'Waiting'}
        </div>
        <div class="list-item-line2" style="display:grid;grid-template-columns:auto 1fr;gap:8px;align-items:start;">
          ...
          <span style="color:#999;">Status:</span><span>${r[4]||'Planned'}</span>
