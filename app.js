...
let shinyMissingSort   = 'rarity'; // 'dex' | 'rarity' | 'az'
...
function renderMissingRareShinies() {
  ...
  const sorted = [...enriched].sort((a, b) => {
    ...
    return a.name.localeCompare(b.name); // dex fallback also alpha for now
  });
  ...
  const renderCard = ({ name, tier, idx, isPending, isConfirmed }) => {
    ...
    const tierBadge = `<div style="font-size:8px;padding:1px 4px;border-radius:3px;background:${tier.badgeBg};color:${tier.badgeText};font-weight:600;">${tier.label}</div>`;
    ...
    return `<div onclick="tapShinyCaught(${idx})"
      ...
      <div style="font-size:10px;font-weight:700;color:#2d3748;line-height:1.2;">${name}</div>
      ${tierBadge}
      ${pendingOverlay}
      ${confirmedOverlay}
    </div>`;
...
function renderHotShinies() {
  ...
  const cards = hot.map(({ name, denom, total, id, have }) => {
    ...
    const haveBadge = have
      ? `<div style="font-size:9px;color:#276749;font-weight:700;">✓ Have it</div>`
      : `<div style="font-size:9px;color:#718096;">🔍 Missing</div>`;

    const detailHtml = isSimple
      ? `<div style="font-size:9px;color:${heatColor};font-weight:600;">${heatLabel}</div>
         ${haveBadge}`
      : `<div style="font-size:11px;font-weight:700;color:${heatColor};">1/${denom}</div>
         <div style="font-size:9px;color:${heatColor};font-weight:600;">${heatLabel}</div>
         ${haveBadge}
         ${lowSample
           ? `<div style="font-size:8px;color:#718096;" title="Small sample">⚠️ n=${total}</div>`
           : `<div style="font-size:8px;color:#999;">n=${total.toLocaleString()}</div>`}`;
