...
function renderWhoMax() {
  ...
  const scored = candidates.map(r => {
    ...
    // Readiness: fraction of total candy already covered (0=nothing done, 1=fully ready)
    const totalNeeded  = needRCraw + needXLraw;
    const totalOwned   = Math.min(speciesCandy, needRCraw) + Math.min(speciesXL, needXLraw);
    const readiness    = totalNeeded > 0 ? totalOwned / totalNeeded : 1;

    // Score: RC still needed + XL still needed (XL weighted 1.5x, harder to get)
    // Lower = better candidate
    const score = needRC + (needXL * 1.5);
  ...
  const surprises = powerUpData
    ...
    .map(r => {
      ...
      const totalOwned   = Math.min(speciesCandy, needRCraw) + Math.min(speciesXL, needXLraw);
      const coverage     = totalOwned / totalNeeded;
      if (coverage < 0.3) return null;   // less than 30% covered overall, not worth surfacing
