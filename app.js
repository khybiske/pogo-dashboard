...
function renderCandy() {
  ...
  for (const r of candidates) {
    ...
    <div class="list-item" draggable="true" data-alloc-pokemon="${pokemon}"
      ...
      <div style="text-align:right;white-space:nowrap;min-width:180px;">
        ${needRC > 0 ? `<div style="display:grid;grid-template-columns:22px 44px 22px 16px auto;align-items:center;gap:2px;justify-items:center;margin-bottom:3px;">
          <button class="btn-icon" style="font-size:10px;padding:1px 4px;width:100%;" onclick="rcAllocAdj('${pokemon}',false,-10)">−</button>
          <input type="number" value="${allocRC}" min="0"
            style="width:100%;text-align:center;font-size:11px;border:1px solid #e2e8f0;border-radius:4px;padding:2px;"
            onchange="rcAllocSet('${pokemon}',false,this.value)">${RC}
          <button class="btn-icon" style="font-size:10px;padding:1px 4px;width:100%;" onclick="rcAllocAdj('${pokemon}',false,10)">+</button>
          <span style="color:#aaa;justify-self:center;">→</span>
          <span style="font-size:11px;font-weight:700;color:${stillRC>0?'#E53E3E':'#48BB78'};text-align:right;width:100%;">${stillRC}${RC}</span>
        </div>` : ''}
        ${needXL > 0 ? `<div style="display:grid;grid-template-columns:22px 44px 22px 16px auto;align-items:center;gap:2px;justify-items:center;">
          <button class="btn-icon" style="font-size:10px;padding:1px 4px;width:100%;" onclick="rcAllocAdj('${pokemon}',true,-10)">−</button>
          <input type="number" value="${allocXL}" min="0"
            style="width:100%;text-align:center;font-size:11px;border:1px solid #e2e8f0;border-radius:4px;padding:2px;"
            onchange="rcAllocSet('${pokemon}',true,this.value)">${RCX}
          <button class="btn-icon" style="font-size:10px;padding:1px 4px;width:100%;" onclick="rcAllocAdj('${pokemon}',true,10)">+</button>
          <span style="color:#aaa;justify-self:center;">→</span>
          <span style="font-size:11px;font-weight:700;color:${stillXL>0?'#E53E3E':'#48BB78'};text-align:right;width:100%;">${stillXL}${RCX}</span>
        </div>` : ''}
...
function renderLuckyWants() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${sorted.map(({r,origIdx}) => {
      ...
      return `
      <div style="background:${cardBg};border-radius:10px;padding:10px 10px 8px;width:110px;
...
function renderMirrorTargets() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${allMirrors.map(m => {
      ...
      return `
      <div style="background:${cardBg};border-radius:10px;padding:10px 10px 8px;width:120px;
                  box-shadow:0 2px 8px rgba(0,0,0,0.08);border:1.5px solid ${cardBorder};
                  display:flex;flex-direction:column;align-items:center;text-align:center;">
    ...
    <div class="list-item" draggable="true" data-index="${idx}" style="${cardBg}align-items:center;">
      ...
      <div style="display:flex;align-items:center;gap:10px;flex:1;min-width:0;flex-wrap:wrap;">
        <div style="min-width:80px;max-width:110px;">
          ...
          ${when  ? `<div style="font-size:10px;color:#718096;margin-top:3px;">📅 ${when}</div>`  : ''}
