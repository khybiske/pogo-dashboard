...
function renderEventChecklist() {
  ...
  for (const se of activeEvents) {
    ...
    rendered.add(evName);
    html += buildEventBlock(evName, endDate, seIdx, clGroups[evName] || []);
  }
...
function buildEventBlock(eventName, endDate, seIdx, items) {
  ...
  if (endDate) {
    ...
    if (!isNaN(d)) endLabel = `<span style="font-size:9px;color:#c05621;font-weight:600;margin-left:6px;">Ends ${d.toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>`;
  }
  ...
  const emptyHint = !totalCount
    ? `<div style="font-size:11px;color:#a0724a;font-style:italic;margin-bottom:8px;">No items yet — hit ➕ Add to build your checklist.</div>`
    : '';
  ...
  // Edit/delete event buttons (only if backed by a SpecialEvents record)
  const eventActions = seIdx >= 0
    ? `<button onclick="editSpecialEvent(${seIdx})"
         style="font-size:10px;padding:2px 6px;border-radius:5px;border:1px solid rgba(194,120,0,0.4);
                background:rgba(255,255,255,0.55);color:#92400e;cursor:pointer;" title="Edit event">✏️</button>
       <button onclick="deleteSpecialEvent(${seIdx})"
         style="font-size:10px;padding:2px 6px;border-radius:5px;border:1px solid rgba(194,120,0,0.4);
                background:rgba(255,255,255,0.55);color:#92400e;cursor:pointer;" title="Delete event">🗑️</button>`
    : '';
    ...
    <div style="margin-top:12px;padding:12px 14px;border-radius:10px;
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:4px;margin-bottom:8px;">
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
          <span style="font-size:13px;font-weight:800;color:#7c2d12;">🎪 ${eventName}</span>
          ${endLabel}
        </div>
        <div style="display:flex;gap:5px;align-items:center;">
          ${eventActions}
          <button onclick="showAddChecklistItemModal('${safeName}','${safeEnd}')"
            style="font-size:10px;padding:2px 8px;border-radius:5px;border:1px solid rgba(194,120,0,0.4);
                   background:rgba(255,255,255,0.55);color:#92400e;cursor:pointer;font-weight:600;">➕ Item</button>
        </div>
      </div>
      ${progressBar}
      ${emptyHint}
