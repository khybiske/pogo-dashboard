...
function renderCandy() {
    ...
    reserveEl.innerHTML = reserveKeys.map(key => {
      ...
      <div class="list-item" style="cursor:default;margin-bottom:4px;">
        ...
        <div class="list-item-content">
          ...
          <div class="list-item-line2">Reserve: <strong>${amount}</strong> <img src="GO_Rare_Candy.png" style="width:14px;height:14px;vertical-align:middle;">${notes ? ' • ' + notes : ''}</div>
        </div>
        <div class="list-item-actions">
  ...
  <div style="font-size:12px;color:#718096;padding:8px 10px;background:#f8f9fa;border-radius:6px;margin-bottom:8px;display:flex;gap:16px;flex-wrap:wrap;align-items:center;">
    ...
    <span>Free: <strong style="color:${summaryColor};">${freeRC} ${RC}</strong></span>
    <span style="color:#888;">│</span>
    <span>${RCX} <strong>${rcXL}</strong> (full pool)</span>
  </div>
  <div style="display:grid;grid-template-columns:auto 1fr auto auto auto;gap:0;align-items:stretch;">
    <div style="display:contents;font-size:10px;color:#999;font-weight:700;text-transform:uppercase;">
      ...
      <div style="padding:4px 6px;text-align:right;">Covers</div>
  ...
  for (const r of candidates) {
    ...
    const coverStr = [
      rcCover > 0 ? `${rcCover}${RC}` : (needRC > 0 ? `0${RC}` : ''),
      xlCover > 0 ? `${xlCover}${RCX}` : (needXL > 0 ? `0${RCX}` : ''),
    ].filter(Boolean).join(' + ') || '—';

    const shortStr = isShort ? [
      rcShort > 0 ? `<span style="color:#E53E3E;">−${rcShort}${RC}</span>` : '',
      xlShort > 0 ? `<span style="color:#E53E3E;">−${xlShort}${RCX}</span>` : '',
  ...
  const footRC = runningRC >= 0
    ? `<span style="color:#48BB78;">${runningRC}${RC} left</span>`
    : `<span style="color:#E53E3E;">−${Math.abs(runningRC)}${RC} short</span>`;
  const footXL = runningXL >= 0
    ? `<span style="color:#48BB78;">${runningXL}${RCX} left</span>`
    : `<span style="color:#E53E3E;">−${Math.abs(runningXL)}${RCX} short</span>`;
