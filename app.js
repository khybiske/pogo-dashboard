...
async function handleSpriteError(img, pokemonName) {
  ...
  img.src = (data && data.sprite) ? data.sprite : PLACEHOLDER_SPRITE;
}
...
function renderMedals() {
  ...
  c.innerHTML = active.map(({ r, idx }) => {
    ...
    <div class="medal-card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:${notes ? 2 : 6}px;">
        <div>
          <span style="font-size:14px;font-weight:700;color:#2d3748;">${name}</span>
          <span class="badge" style="background:#f0eeff;color:#553c9a;border:1px solid #d6c9ff;margin-left:6px;font-size:10px;">${tierLabel}</span>
        </div>
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <button class="btn-icon" style="font-size:10px;padding:1px 5px;" onclick="editMedal(${idx})" title="Edit">✏️</button>
          <button class="btn-icon" style="font-size:10px;padding:1px 5px;" onclick="deleteMedal(${idx})" title="Delete">🗑️</button>
        </div>
      </div>
      ${notes ? `<div style="font-size:11px;color:#718096;margin-bottom:6px;font-style:italic;">${notes}</div>` : ''}
      <div style="display:flex;height:14px;gap:2px;border-radius:7px;overflow:hidden;">
        ${seg(bPct,  fillB,  '#cd7f32', '#f0e4d0', 7, 0)}
        ${seg(sPct,  fillS,  '#9e9e9e', '#ebebeb', 0, 0)}
        ${seg(gPct,  fillG,  '#f6c12a', '#fdf3cc', 0, 0)}
        ${seg(plPct, fillPl, '#63e5ff', '#d9f7ff', 0, 7)}
      </div>
      <div style="display:flex;justify-content:space-between;margin-top:5px;font-size:10px;color:#718096;">
        <span style="color:#2d3748;font-weight:600;">▲ ${remaining.toLocaleString()} to ${nextLabel}</span>
        <span>${bronze.toLocaleString()} · ${silver.toLocaleString()} · ${gold.toLocaleString()} · ${platinum.toLocaleString()}</span>
      </div>
    </div>`;
  }).join('');
}
...
const SECTION_LABELS = {
  ...
  candy:     '🍬 Rare Candy',
  lucky:     '🤝 Lucky Trade Management',
  shinygaps: '✨ Community Day Shiny Gaps',
  shinyodds: '✨ Shiny Odds Reference',
  pvp:       '⚔️ PVP Roster',
  medals:    '🏅 Medal Progress',
};
const DEFAULT_ORDER = ['events','raids','powerup','candy','lucky','shinygaps','shinyodds','pvp','medals'];
...
async function loadAllData() {
  ...
  const [pu, candy, wants, mirrors, friends, shinies, events, pvpRoster, pvpConfig, pvpMeta, raids, pvpRosterUL, pvpMetaUL, medalsRaw] = await Promise.all([
    ...
    safe('PVPRosterUL'), safe('PVPMetaUL'), safe('Medals')
  ]);
...
function renderAll() {
  for (const [fn, name] of [[renderEvents,'Events'],[renderRaidBosses,'Raids'],[renderPowerUp,'PowerUp'],[renderCandy,'Candy'],[renderTrades,'Trades'],[renderShinyGaps,'Shinies'],[renderPVP,'PVP'],[renderShinyOdds,'ShinyOdds'],[renderMedals,'Medals']]) {
...
// ── Raid Boss Tracker ─────────────────────────────────────────────────────────
const RAID_HEADERS = ['Pokemon','Tier','Shiny','IVs','XL','Dex','Costume','Priority','Notes','Done','DoneDate','Remote','Shadow'];
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:10px;">
    ${sorted.map(({r,idx}) => {
      ...
      const remoteRibbon = (!done && isRemote)
        ...
        : '';
      const cardBorder = done ? '#48BB78' : isRemote ? '#00b4db' : tierColor;

      // Build sprite slug: mega → append -mega; gmax → append -gmax
      ...
      <div style="position:relative;display:flex;flex-direction:column;align-items:center;
                  width:90px;background:${isShadow ? 'linear-gradient(160deg,rgba(107,70,193,0.12) 0%,white 60%)' : 'white'};
...
function raidSpritePreview() {
  ...
  raidSpriteTimer = setTimeout(async () => {
    ...
    // Build the correct PokeAPI slug based on tier
    let apiSlug;
    if (tier === 'Mega') {
      // Charizard/Mewtwo have X/Y — default to X (user can edit name to specify)
    ...
    if (spriteUrl) {
      img.src = spriteUrl;
      info.textContent = tier === 'Mega' ? '✓ Mega form' : tier === 'GMAX' ? '✓ Gigantamax form' : '✓';
      info.style.color = '#48BB78';
    } else {
      // Fallback: show base sprite with a note
...
function renderShinyGaps() {
  ...
  c.innerHTML = '<div class="shiny-grid">'+sorted.map(({r,idx})=>{
    ...
    return `
    <div class="${cardClass} card-shiny" data-idx="${idx}" onclick="cycleShinyPriority(${idx})" style="cursor:pointer;position:relative;overflow:hidden;" title="Click to cycle: Want → High Priority → Got it!">
...
async function cycleShinyPriority(idx) {
  ...
  // Instant visual feedback — find card by data-idx
  const clickedCard = document.querySelector(`.shiny-card[data-idx="${idx}"]`);
  if (clickedCard) {
    clickedCard.style.transform = 'scale(0.92)';
  ...
  // Re-render immediately so UI updates without waiting for network
  renderShinyGaps();
