...
async function loadAllData() {
  ...
  // Fetch shinyrates.com in parallel — separate cache with 30-min TTL
  const fetchShinyRates = async () => {
    try {
      ...
      const r = await fetch('https://shinyrates.com/data/rate');
      const j = await r.json();
      shinyRatesData = j;
...
function renderShinyGaps() {
  ...
  const hasCDColumn = shinyGapsData.some(r => (r[3]||'').toUpperCase() === 'TRUE');
  const cdRows = hasCDColumn
    ? shinyGapsData.map((r,idx)=>({r,idx})).filter(({r}) => (r[3]||'').toUpperCase() === 'TRUE')
