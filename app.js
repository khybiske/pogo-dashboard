...
async function confirmShinyCaught(idx, evt) {
  ...
  renderMissingRareShinies();
  setBgSaving(true);
  await saveSheetData('ShinyCollection', [['Pokemon','Have','Priority','CD'], ...shinyGapsData]);
  setBgSaving(false);
...
function renderMissingRareShinies() {
  ...
  const ratesMap = shinyRatesData.length ? buildRatesLookup() : {};

  // All rows — include recently-caught ones still in grace period
  const displayRows = shinyGapsData.map((r, idx) => ({ r, idx })).filter(({ r, idx }) => {
    ...
    return !isCaught || shinyMissingCaught.has(idx);
  });
  ...
  // Enrich with rate + dex id for sorting
  const enriched = displayRows.map(({ r, idx }) => {
    const name  = (r[0] || '').trim();
    const entry = ratesMap[name.toLowerCase()];
    const dex   = entry?.id || 99999;
    const denom = (entry && entry.denom !== null) ? entry.denom : null;
    const total = entry?.total || 0;
    return { name, dex, denom, total, id: entry?.id || null, idx,
  ...
  const sorted = [...enriched].sort((a, b) => {
    if (shinyMissingSort === 'rarity') {
      if (a.denom === null && b.denom === null) return a.name.localeCompare(b.name);
      if (a.denom === null) return 1;
      if (b.denom === null) return -1;
      return b.denom - a.denom;
    }
    if (shinyMissingSort === 'az') return a.name.localeCompare(b.name);
    return a.dex - b.dex; // default: pokédex #
  });

  const tiers = [
    { min: 1000, label: 'Ultra Rare', color: '#6b46c1', badgeBg: 'rgba(107,70,193,0.12)', badgeText: '#553c9a' },
    { min: 600,  label: 'Very Rare',  color: '#3182ce', badgeBg: 'rgba(49,130,206,0.12)', badgeText: '#2b6cb0' },
    { min: 300,  label: 'Rare',       color: '#38a169', badgeBg: 'rgba(56,161,105,0.12)', badgeText: '#276749' },
    { min: 0,    label: 'Common',     color: '#718096', badgeBg: 'rgba(113,128,150,0.10)', badgeText: '#4a5568' },
  ];
  const getTier = d => tiers.find(t => d >= t.min) || tiers[tiers.length - 1];
  const shinyGrad = 'background:linear-gradient(135deg,rgba(255,236,153,0.35) 0%,rgba(255,255,255,0) 60%,rgba(255,215,100,0.2) 100%)';

  const renderCard = ({ name, dex, denom, total, id, idx, isPending, isConfirmed }) => {
    const spriteUrl   = id
      ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/${id}.png`
      : sprite(name, null, null, true);
    const fallbackUrl = id
      ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png`
      : '';
    const lowSample   = total > 0 && total < 200;
    const t           = denom !== null ? getTier(denom) : null;

    const borderStyle = isConfirmed ? 'border:2px solid #48BB78;'
                      : isPending   ? 'border:2px solid #6b46c1;'
                      : `border:1px solid ${t ? t.color + '18' : '#e2e8f0'};`;

    // First tap: "Got it?" with ✓ confirm and ↩ cancel buttons
    ...
    // Confirmed: brief green ✓ before auto-dismiss
    ...
    const rateHtml = isSimple
      ? (denom !== null
          ? `<div style="font-size:8px;padding:1px 4px;border-radius:3px;background:${t.badgeBg};color:${t.badgeText};font-weight:600;">${t.label}</div>`
          : '')
      : (denom !== null
          ? `<div style="font-size:9px;font-weight:600;color:${t.color};">1/${denom.toLocaleString()}</div>
             <div style="font-size:8px;padding:1px 4px;border-radius:3px;background:${t.badgeBg};color:${t.badgeText};font-weight:600;">${t.label}</div>
             ${lowSample ? `<div style="font-size:8px;color:#C05621;" title="Low sample">⚠️ n=${total}</div>` : ''}`
          : `<div style="font-size:8px;color:#aaa;">No rate data</div>`);
    ...
    return `<div onclick="tapShinyCaught(${idx})"
      ...
      <img src="${spriteUrl}" style="width:40px;height:40px;"
           ${fallbackUrl ? `onerror="this.src='${fallbackUrl}'"` : `onerror="handleSpriteError(this,'${name}')"`}>
  ...
  const ratedCount   = enriched.filter(e => e.denom !== null).length;

  // Simple mode: cap display at 30 cards
  const SIMPLE_CUTOFF = 30;
  const displaySorted = (isSimple && sorted.length > SIMPLE_CUTOFF)
    ? sorted.slice(0, SIMPLE_CUTOFF)
    : sorted;
  ...
  c.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px;margin-bottom:8px;">
      <div style="font-size:11px;color:#999;">${missingCount} missing · ${ratedCount} with live rate data</div>
...
function renderHotShinies() {
  ...
  const cards = hot.map(({ name, denom, total, id, have }) => {
    ...
    // Green border for caught, heat border otherwise
