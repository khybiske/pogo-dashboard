...
async function handleSpriteError(img, pokemonName) {
  ...
  if (data && (data.sprite || data.homeSprite)) {
    img.src = data.shinySprite || data.sprite || data.homeSprite;
    return;
  }
  // If the slug had a form suffix and failed, retry with base name
  const baseMatch = resolvedSlug.match(/^(.+?)-(mega|mega-x|mega-y|gmax|origin|alola|galar|hisui|paldea)$/);
  if (baseMatch) {
    const baseSlug = POKEAPI_SLUG_OVERRIDES[baseMatch[1]] || baseMatch[1];
...
function renderLuckyWants() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${all.map(m => {
      ...
      const badges = [m.shiny?'✨':'', m.legacy?'£':'', m.dmax?'💎':'', m.gmax?'🌟':'',
        m.background?'🖼️':'', m.costume?'🎭':'', m.legendary?'🏆':'', m.purified?'🤍':'', m.isNew?'🆕':'',
      ].filter(Boolean).join(' ');
...
function renderMirrorTargets() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${allMirrors.map(m => {
      ...
      const badgeIcons = [m.shiny?'✨':'', m.legacy?'£':'', m.dmax?'💎':'', m.gmax?'🌟':'', m.background?'🖼️':'', m.costume?'🎭':'', m.legendary?'🏆':'', m.purified?'🤍':''].filter(Boolean).join(' ');
