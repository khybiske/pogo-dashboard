...
// Helper functions
// Stardust cost to level mapping (for normal Pokemon)
const STARDUST_TO_LEVEL = {
  200: [1, 2], 400: [2.5, 3.5], 600: [4, 5], 800: [5.5, 6.5], 1000: [7, 8],
  1300: [8.5, 9.5], 1600: [10, 11], 1900: [11.5, 12.5], 2200: [13, 14], 2500: [14.5, 15.5],
  3000: [16, 17], 3500: [17.5, 18.5], 4000: [19, 20], 4500: [20.5, 21.5], 5000: [22, 23],
  6000: [23.5, 24.5], 7000: [25, 26], 8000: [26.5, 27.5], 9000: [28, 29], 10000: [29.5, 30.5],
  11000: [31, 32], 12000: [32.5, 33.5], 13000: [34, 35], 14000: [35.5, 36.5], 15000: [37, 38]
};

function calculateLevelFromStardust() {
  const stardust = parseInt(document.getElementById('puStardust').value) || 0;
  ...
  const actualCost = isLucky ? stardust * 2 : stardust;
  
  if (STARDUST_TO_LEVEL[actualCost]) {
    const [minLv, maxLv] = STARDUST_TO_LEVEL[actualCost];
    levelField.value = minLv;
    helperText.textContent = `💡 Level ${minLv}-${maxLv} (${isLucky ? 'Lucky cost' : 'Normal cost'})`;
    ...
    <div class="list-item" draggable="true" data-index="${i}" style="background:${typesBg};${isArch?'opacity:0.5;':''}">
      ...
      <div class="list-item-content">
        ...
        <div class="list-item-line2">
          ${types_html}${types_html?' •':''} Lv${curLv}→${tgtLv}${ivDisplay} • ${role} • ${status}${notes?' • '+notes:''}
        </div>
      </div>
...
async function savePowerUp(e) {
  ...
  const ok = await saveSheetData('PowerUpPriority',[PU_HEADERS,...powerUpData]);
