...
let pvpRosterData=[], pvpConfigData=[];
...
async function loadAllData() {
  ...
  const [pu, candy, wants, mirrors, friends, shinies, events, pvpRoster, pvpConfig] = await Promise.all([
...
function renderPVP() {
  ...
  // Sort by PVPpoke rank ascending (blank/0 goes to end)
  const sorted = pvpRosterData
    .map((r, idx) => ({r, idx}))
    .sort((a, b) => {
      const ra = parseInt(a.r[1]) || 9999;
      const rb = parseInt(b.r[1]) || 9999;
      return ra - rb;
    });
