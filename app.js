...
async function loadAllData() {
  ...
  const [pu, candy, wants, mirrors, friends, shinies, events, pvpRoster, pvpConfig, pvpMeta, raids, pvpRosterUL, pvpMetaUL, medalsRaw, upcomingCDsRaw] = await Promise.all([
    ...
    safe('PVPRosterUL'), safe('PVPMetaUL'), safe('Medals'), safe('UpcomingCDs')
  ]);
...
function renderAll() {
  for (const [fn, name] of [[renderEvents,'Events'],[renderRaidBosses,'Raids'],[renderPowerUp,'PowerUp'],[renderCandy,'Candy'],[renderTrades,'Trades'],[renderShinyGaps,'Shinies'],[renderPVP,'PVP'],[renderShinyOdds,'ShinyOdds'],[renderMedals,'Medals'],[renderUpcomingCDs,'UpcomingCDs']]) {
...
function renderUpcomingCDs() {
  ...
  const sorted = [...upcomingCDsData]
    .map((r, idx) => ({ r, idx }))
    .sort((a, b) => {
      const order = { 'Confirmed': 0, 'Rumored': 1 };
  ...
  c.innerHTML = '<div class="shiny-grid">' + sorted.map(({ r, idx }) => {
    ...
    const badgeBg    = confirmed ? '#c6f6d5' : '#fef3c7';
    const badgeColor = confirmed ? '#276749' : '#92400e';
    const badgeIcon  = confirmed ? '✅' : '🔮';
    const cardBorder = confirmed ? '#48BB78' : '#D69E2E';

    return `
    <div class="shiny-card" style="border:2px solid ${cardBorder};position:relative;cursor:default;" title="${notes || pokemon}">
      <img src="${sprite(r[0], null, null, true)}" class="shiny-sprite" onerror="handleSpriteError(this,'${r[0]}')" draggable="false">
      ...
      <div style="display:inline-block;background:${badgeBg};color:${badgeColor};font-size:8px;font-weight:700;
                  padding:1px 5px;border-radius:8px;margin-top:2px;">${badgeIcon} ${status}</div>
      ...
      <div style="display:flex;gap:3px;justify-content:center;margin-top:4px;">
        <button class="btn-icon" style="font-size:9px;padding:1px 3px;" onclick="editUpcomingCD(${idx})" title="Edit">✏️</button>
        <button class="btn-icon" style="font-size:9px;padding:1px 3px;" onclick="deleteUpcomingCD(${idx})" title="Delete">🗑️</button>
      </div>
    </div>`;
  }).join('') + '</div>';
}
