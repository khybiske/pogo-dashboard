...
// Trades
const WANTS_HEADERS  = ['Pokemon','Reason','Priority','Notes','Shiny','Legacy','DMAX','GMAX','Background','Costume','Legendary','Purified'];
...
function renderLuckyWants() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${sorted.map(({r,origIdx}) => {
      ...
      const special    = isSpecialTrade({ shiny, legendary, costume, purified, gmax, background });
      ...
      const badges = [
        ...
        background?'🖼️':'', costume?'🎭':'', legendary?'🏆':'', purified?'🤍':'',
      ].filter(Boolean).join(' ');
...
function isSpecialTrade({ shiny, legendary, costume, purified, gmax, background }) {
  return !!(shiny || legendary || costume || purified || gmax || background);
}
...
const SPECIAL_TRADE_BADGE = `<span style="display:inline-flex;align-items:center;gap:3px;font-size:9px;font-weight:700;
  ...
  box-shadow:0 1px 4px rgba(253,160,133,0.4);" title="Special Trade — extra Stardust cost">⭐ Special Trade</span>`;
...
function lwRefreshBadge() {
  updateSpecialTradeBadge('lwSpecialBadge', {shiny:'lwShiny',legendary:'lwLegendary',costume:'lwCostume',purified:'lwPurified',gmax:'lwGmax',background:'lwBackground'});
}
