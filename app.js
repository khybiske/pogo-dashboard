...
function renderPVP() {
    ...
    if (!pvpMetaData.length) {
      ...
      metaEl.innerHTML = `
        <div style="font-size:11px;color:#718096;margin-bottom:8px;">
          ...
          <span style="color:#999;">dimmed = not in your roster</span>
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;">
          ${metaSorted.map(r => {
            ...
            <div style="display:flex;flex-direction:column;align-items:center;width:58px;
                        opacity:${have?1:0.35};filter:${have?'none':'grayscale(0.6)'};">
              <div style="font-size:9px;font-weight:700;color:${have?'#553c9a':'#999'};margin-bottom:2px;">#${rank}</div>
              <img src="${sprite(pokemon)}" onerror="handleSpriteError(this,'${pokemon}')"
                   style="width:40px;height:40px;background:${have?'rgba(85,60,154,0.1)':'#f0f0f0'};
                          border-radius:6px;padding:2px;border:1px solid ${have?'#b794f4':'#e2e8f0'};">
              <div style="font-size:9px;text-align:center;color:${have?'#2d3748':'#999'};
              ...
              ${have ? '<div style="font-size:9px;color:#48BB78;">✓</div>' : '<div style="font-size:9px;color:#ccc;">✗</div>'}
            </div>`;
          }).join('')}
        </div>`;
    }
  ...
  // Build meta rank lookup: pokemon name → rank number
  const metaRankMap = {};
  ...
  c.innerHTML = sorted.map(({r, idx}) => {
    ...
    const readyStyle = ready
      ? 'border-left-color:#48BB78;background:rgba(72,187,120,0.06);'
    ...
    return `
    <div class="list-item" style="${readyStyle}align-items:center;">
      <img class="list-item-sprite" src="${sprite(pokemon)}" onerror="handleSpriteError(this,'${pokemon}')">
      <div class="list-item-content">
