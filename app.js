...
function renderWhoMax() {
  ...
  const surprises = powerUpData
    ...
    .map(r => {
      ...
      const needXLraw    = candy.xlCandy;
      const needRCraw    = candy.regularCandy;
      if (needXLraw === 0) return null; // no XL needed, not interesting
      const xlCoverage = speciesXL / needXLraw; // 0–1+
      if (xlCoverage < 0.5) return null;        // less than half covered, not worth surfacing
      const needXL  = Math.max(0, needXLraw - speciesXL);
      const needRC  = Math.max(0, needRCraw - speciesCandy);
      return { r, pokemon, curLv, speciesXL, speciesCandy, regional, form, isShiny, isShadow,
               needXLraw, needRCraw, needXL, needRC, xlCoverage };
    })
    .filter(Boolean)
    .sort((a, b) => b.xlCoverage - a.xlCoverage)
    .slice(0, 2);
  ...
  if (surprises.length) {
    c.innerHTML += `<div style="font-size:11px;font-weight:700;color:#553c9a;text-transform:uppercase;
      ...
      💡 Worth considering — XL candy rich, but no L50 goal set yet</div>`;

    c.innerHTML += surprises.map(({ r, pokemon, curLv, speciesXL, speciesCandy, regional, form,
                                    isShiny, isShadow, needXLraw, needRCraw, needXL, needRC, xlCoverage }) => {
      let displayName = pokemon;
      if (regional) displayName = `${regional} ${pokemon}`;
      if (form)     displayName = `${displayName} (${form})`;
      ...
      const ivHtml    = iv ? `<span style="font-size:10px;color:#667eea;font-weight:600;">${iv}% IV</span>` : '';
      const xlPct     = Math.round(xlCoverage * 100);
      ...
      <div class="whomax-card" style="display:grid;grid-template-columns:52px 1fr;gap:10px;align-items:start;
        ...
        <div style="display:flex;flex-direction:column;align-items:center;gap:3px;">
          <img src="${sprite(pokemon, regional, form, isShiny)}" onerror="handleSpriteError(this,'${pokemon}')"
               style="width:48px;height:48px;">
          <span style="font-size:9px;color:#553c9a;font-weight:700;text-align:center;line-height:1.2;">💡 ${xlPct}% XL ready</span>
        </div>
        <div style="min-width:0;">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;flex-wrap:wrap;">
            <span style="font-size:13px;font-weight:700;">${displayName}</span>
          ...
          <div class="whomax-progress" style="background:#e2e8f0;border-radius:4px;height:5px;margin-bottom:6px;overflow:hidden;">
            <div style="height:100%;width:${xlPct}%;background:#553c9a;border-radius:4px;"></div>
          </div>
