...
function getTypeBackground(typesStr) {
  ...
  if (types.length === 1) {
    return `var(--type-${types[0]}-bg, rgba(200,200,200,0.08))`;
  } else {
    const type1 = `var(--type-${types[0]}-bg, rgba(200,200,200,0.08))`;
    const type2 = `var(--type-${types[1]}-bg, rgba(200,200,200,0.08))`;
    return `linear-gradient(90deg, ${type1} 0%, ${type2} 100%)`;
  }
}
...
const PU_HEADERS = ['Pokemon','Types','Lucky','Has Elite','Needs Elite','Current Level',
                    ...
                    'Priority','Status','Archived','Hundo','Shiny','Shadow'];
...
function renderPowerUp() {
  ...
  c.innerHTML = rows.map(({r,i}) => {
    const [pokemon,types,lucky,hasEl,needEl,curLv,tgtLv,role,,,needLucky,notes,prio,status,arch,hundo,shiny,shadow] = r;
    ...
    <div class="list-item" draggable="true" data-index="${i}" style="background:${typesBg};${isArch?'opacity:0.5;':''}">
      ...
      <div class="list-item-content">
        ...
        <div class="list-item-line2">
          ${types_html}${types_html?' •':''} Lv${curLv}→${tgtLv} • ${role} • ${status}${notes?' • '+notes:''}
        </div>
      </div>
