...
function renderEvents() {
  ...
  const gridCols = DAYS.map((day, i) => {
    ...
    const colBg    = isToday ? 'rgba(102,126,234,0.05)' : isPast ? 'rgba(0,0,0,0.02)' : 'transparent';

    return `<div style="min-width:0;background:${colBg};border-radius:8px;overflow:hidden;">
...
function renderPowerUp() {
  ...
  c.innerHTML = rows.map(({r,i}) => {
    ...
    // Level 50 indicators
    if (curLevel >= 50) {
      b.push('<span class="badge badge-complete50">L50 ✓</span>');
    } else if (tgtLevel >= 50) {
    ...
    <div class="list-item" draggable="true" data-index="${i}" style="background:${typesBg};${isArch||targetMet?'opacity:0.55;':''}">
      ...
      <div class="list-item-content">
        ...
        <div class="list-item-line2">
          ${types_html}${types_html?' •':''} Lv${curLv}→${tgtLv}${ivDisplay}${candyDisplay}${notes?' • '+notes:''} • ${status}
        </div>
      </div>
