...
function renderCandy() {
    ...
    reserveEl.innerHTML = reserveKeys.map(key => {
      ...
      <div class="list-item" style="cursor:default;margin-bottom:4px;">
        ...
        <div class="list-item-content">
          ...
          <div class="list-item-line2">Reserve: <strong>${amount} RC</strong>${notes ? ' • ' + notes : ''}</div>
        </div>
        <div class="list-item-actions">
  ...
  const freeRC  = Math.max(0, rcRegular - totalReserved);

  // Filter power-up entries: Legendary or Mythical AND target level = 50
  ...
  let runningRC = freeRC;
  let runningXL = rcXL;

  // Summary header
  const summaryColor = freeRC < 0 ? '#E53E3E' : '#48BB78';
  let html = `
  <div style="font-size:12px;color:#718096;padding:8px 10px;background:#f8f9fa;border-radius:6px;margin-bottom:8px;display:flex;gap:16px;flex-wrap:wrap;">
    <span>Total RC: <strong>${rcRegular}</strong></span>
    <span>Reserves: <strong style="color:#C05621;">−${totalReserved}</strong></span>
    <span>Free to allocate: <strong style="color:${summaryColor};">${freeRC} RC</strong></span>
    <span style="color:#888;">│</span>
    <span>XL RC: <strong>${rcXL}</strong> (full pool, no reserves)</span>
  </div>
  <div style="display:grid;grid-template-columns:auto 1fr auto auto auto;gap:0;align-items:stretch;">
    <div style="display:contents;font-size:10px;color:#999;font-weight:700;text-transform:uppercase;">
      ...
      <div style="padding:4px 6px;text-align:right;">Needs</div>
      <div style="padding:4px 6px;text-align:right;">RC covers</div>
      <div style="padding:4px 6px;text-align:right;">Remaining</div>
    </div>`;
  ...
  for (const r of candidates) {
    ...
    // How much RC can we cover
    const rcCover   = Math.min(needRC, runningRC);
    const xlCover   = Math.min(needXL, runningXL);
    runningRC      -= rcCover;
    runningXL      -= xlCover;

    const rcShort  = needRC - rcCover;
    const xlShort  = needXL - xlCover;
    const isShort  = rcShort > 0 || xlShort > 0;
    const rowBg    = isShort ? 'background:rgba(229,62,62,0.04);' : '';
    ...
    const coverStr = [
      rcCover > 0 ? `${rcCover}🍬` : (needRC > 0 ? '0🍬' : ''),
      xlCover > 0 ? `${xlCover}🍬XL` : (needXL > 0 ? '0🍬XL' : ''),
    ].filter(Boolean).join(' + ') || '—';

    const shortStr = isShort ? [
      rcShort > 0 ? `<span style="color:#E53E3E;">−${rcShort}🍬</span>` : '',
      xlShort > 0 ? `<span style="color:#E53E3E;">−${xlShort}🍬XL</span>` : '',
    ].filter(Boolean).join(' ') : `<span style="color:#48BB78;">✓</span>`;
    ...
    <div style="display:contents;">
      ...
      <div style="padding:6px 6px;${rowBg}border-top:1px solid #f0f0f0;font-size:11px;text-align:right;color:#4a5568;">${needStr}</div>
      <div style="padding:6px 6px;${rowBg}border-top:1px solid #f0f0f0;font-size:11px;text-align:right;color:#48BB78;">${coverStr}</div>
      <div style="padding:6px 6px;${rowBg}border-top:1px solid #f0f0f0;font-size:11px;text-align:right;">${shortStr}</div>
    </div>`;
  }

  // Running totals footer
  const footRC = runningRC >= 0
    ? `<span style="color:#48BB78;">${runningRC} RC left over</span>`
    : `<span style="color:#E53E3E;">${Math.abs(runningRC)} RC short</span>`;
  const footXL = runningXL >= 0
    ? `<span style="color:#48BB78;">${runningXL} XL left over</span>`
    : `<span style="color:#E53E3E;">${Math.abs(runningXL)} XL short</span>`;

  html += `
    <div style="display:contents;">
      <div style="padding:8px 4px;border-top:2px solid #e2e8f0;"></div>
      <div style="padding:8px 6px;border-top:2px solid #e2e8f0;font-size:11px;color:#718096;font-style:italic;">After all allocations</div>
      <div style="padding:8px 6px;border-top:2px solid #e2e8f0;"></div>
      <div style="padding:8px 6px;border-top:2px solid #e2e8f0;"></div>
      <div style="padding:8px 6px;border-top:2px solid #e2e8f0;font-size:11px;text-align:right;">${footRC}<br>${footXL}</div>
    </div>
  </div>`;
...
function renderLuckyFriends() {
  ...
  c.innerHTML = luckyFriendsData.map((r,idx)=>{
    ...
    // Status badge: Planned=green, Waiting=amber, Complete=grey
    ...
    // Pokemon slot - sprite + label + name + badges all in one compact column
    function pokeSlot(label, name, shiny, dmax, gmax) {
      const badges = [shiny?'✨':'', dmax?'💎':'', gmax?'🌟':''].filter(Boolean).join('');
      if (!name) return `
        <div style="display:flex;flex-direction:column;align-items:center;min-width:54px;opacity:0.3;">
          <div style="font-size:9px;color:#999;font-weight:600;text-transform:uppercase;letter-spacing:0.3px;margin-bottom:2px;">${label}</div>
          <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/0.png"
               style="width:38px;height:38px;filter:grayscale(1);">
          <div style="font-size:10px;color:#bbb;">TBD</div>
        </div>`;
      return `
        <div style="display:flex;flex-direction:column;align-items:center;min-width:54px;">
          <div style="font-size:9px;color:#667eea;font-weight:700;text-transform:uppercase;letter-spacing:0.3px;margin-bottom:2px;">${label}</div>
          <img src="${sprite(name)}" onerror="handleSpriteError(this,'${name}')"
               style="width:38px;height:38px;${shiny?'filter:drop-shadow(0 0 4px gold);':''}">
          <div style="font-size:10px;font-weight:600;color:#2d3748;text-align:center;max-width:70px;line-height:1.2;">${name}</div>
          ${badges ? `<div style="font-size:11px;line-height:1;">${badges}</div>` : ''}
        </div>`;
    }
