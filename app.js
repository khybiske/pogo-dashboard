...
function sprite(name, regional, form, shiny) {
  ...
  const cacheKey = `pokemon_data_v2_${apiSlug}`;
...
async function getPokemonData(name) {
  ...
  // Check localStorage cache
  const cacheKey = `pokemon_data_v2_${cleanName}`;
...
function renderRaidBosses() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:10px;">
    ${sorted.map(({r,idx}) => {
      ...
      // Build sprite slug: mega tier → append -mega; Charizard/Mewtwo have X/Y variants default to X
      const baseSlug = pokemon.toLowerCase().replace(/\s+/g, '-');
      let spriteSlug = baseSlug;
      if (tier === 'Mega') {
        // Special two-form megas default to X
