...
// Data arrays
let powerUpData=[], candyData=[], tradePlansData=[], shinyGapsData=[], eventsData=[], eventChecklistData=[];
...
const ALL_SHEETS = [
  ...
  'PVPRosterUL','PVPMetaUL','Medals','UpcomingCDs','UsefulLinks','EventChecklist'
];
...
function renderAll() {
  for (const [fn, name] of [[renderEvents,'Events'],[renderRaidBosses,'Raids'],[renderPowerUp,'PowerUp'],[renderWhoMax,'WhoMax'],[renderCandy,'Candy'],[renderTrades,'Trades'],[renderMissingRareShinies,'MissingRare'],[renderHotShinies,'HotShinies'],[renderPVP,'PVP'],[renderShinyOdds,'ShinyOdds'],[renderMedals,'Medals'],[renderUpcomingCDs,'UpcomingCDs'],[renderLinks,'Links'],[renderEventChecklist,'EventChecklist']]) {
...
// Events
// ── Event Checklist ────────────────────────────────────────────────────────────
// Schema: ['EventName','Item','Day','Done']  (columns 0–3)
// ── Event Checklist ────────────────────────────────────────────────────────────
// Schema: ['EventName','Item','Day','Done','EndDate']  (columns 0–4)
// EndDate = YYYY-MM-DD; event group auto-hides 1 day after EndDate
...
function checklistIsExpired(endDate) {
  ...
  // Hide starting the day after EndDate (so the day-of is still shown)
...
function renderEventChecklist() {
  ...
  if (!eventChecklistData.length) { c.innerHTML = ''; return; }

  // Group by EventName, skip expired groups
  const groups = {};
  const groupOrder = [];
  eventChecklistData.forEach((r, idx) => {
    const eventName = (r[0] || '').trim() || 'General';
    const endDate   = (r[4] || '').trim();
    if (checklistIsExpired(endDate)) return;
    if (!groups[eventName]) { groups[eventName] = { items: [], endDate }; groupOrder.push(eventName); }
    groups[eventName].items.push({ r, idx });
    if (endDate && (!groups[eventName].endDate || endDate > groups[eventName].endDate)) {
      groups[eventName].endDate = endDate;
    }
  });

  if (!groupOrder.length) { c.innerHTML = ''; return; }

  const blocksHtml = groupOrder.map(eventName => {
    const { items, endDate } = groups[eventName];
    ...
    let endLabel = '';
    if (endDate) {
      const d = new Date(endDate + 'T12:00:00'); // noon to avoid timezone off-by-one
      if (!isNaN(d)) {
        endLabel = `<span style="font-size:9px;color:#c05621;font-weight:600;margin-left:6px;">Ends ${d.toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>`;
      }
    }
        ...
        return `
          <div style="display:flex;align-items:center;gap:6px;padding:6px 8px;border-radius:7px;
            ...
            <div style="display:flex;gap:2px;flex-shrink:0;opacity:0.45;">
              <button class="btn-icon" style="font-size:9px;padding:2px;" onclick="event.stopPropagation();editChecklistItem(${idx})" title="Edit">✏️</button>
              <button class="btn-icon" style="font-size:9px;padding:2px;" onclick="event.stopPropagation();deleteChecklistItem(${idx})" title="Delete">🗑️</button>
            </div>
          </div>`;
      }).join('');
      ...
      <div style="margin-top:12px;padding:12px 14px;border-radius:10px;
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:4px;margin-bottom:8px;">
          ...
          <div style="display:flex;gap:5px;">
            <button onclick="showAddChecklistItemModal('${safeName}','${endDate}')"
              style="font-size:10px;padding:2px 8px;border-radius:5px;border:1px solid rgba(194,120,0,0.4);
                     background:rgba(255,255,255,0.55);color:#92400e;cursor:pointer;font-weight:600;">➕ Add</button>
            <button onclick="clearAllChecklistItems('${safeName}')"
              style="font-size:10px;padding:2px 8px;border-radius:5px;border:1px solid rgba(194,120,0,0.4);
                     background:rgba(255,255,255,0.55);color:#92400e;cursor:pointer;font-weight:600;">↺ Reset</button>
          </div>
        </div>
        ${progressBar}
        ${dayHtml}
  ...
  c.innerHTML = blocksHtml + `
    <div style="text-align:right;margin-top:6px;">
      <button onclick="showAddChecklistItemModal()"
        style="font-size:10px;padding:2px 8px;border-radius:5px;border:1px solid #cbd5e0;
               background:white;color:#718096;cursor:pointer;">➕ New event checklist</button>
    </div>`;
}
...
function showAddChecklistItemModal(prefillEvent='', prefillEndDate='') {
  ...
  const names = [...new Set(eventChecklistData.map(r => (r[0]||'').trim()).filter(Boolean))];
...
async function saveChecklistItem(e) {
  ...
  // Propagate EndDate to all existing items with the same event name that don't have one yet
  if (endDate && evName) {
...
async function clearAllChecklistItems(eventNameFilter='') {
  if (!eventChecklistData.length) return;
  const label = eventNameFilter ? `"${eventNameFilter}"` : 'all events';
  if (!confirm(`Reset checklist for ${label}?`)) return;
  setBgSaving(true);
  eventChecklistData.forEach(r => {
    if (!eventNameFilter || (r[0]||'').trim() === eventNameFilter) r[3] = 'FALSE';
  });
  await saveSheetData('EventChecklist', [CHECKLIST_HEADERS, ...eventChecklistData.map(r => padRow(r, CHECKLIST_HEADERS.length))]);
  setBgSaving(false);
  renderEventChecklist();
