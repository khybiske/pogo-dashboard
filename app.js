...
function renderCandy() {
  ...
  for (const r of candidates) {
    ...
    const speciesXL  = parseInt(r[8]) || 0;   // XL candy already owned for this species
    ...
    const needXL     = Math.max(0, needXLRaw - speciesXL); // species XL offsets RC XL needed
...
function renderRaidBosses() {
  ...
  // Auto-reset: if DoneDate was before last Monday, clear Done flag
  const lastMonday = getLastMonday();
  let changed = false;
  raidBossData.forEach(r => {
    if (isTrue(r[9]) && r[10]) {
      const doneDate = new Date(r[10]);
      if (doneDate < lastMonday) { r[9] = 'FALSE'; r[10] = ''; changed = true; }
    }
  });
  ...
  c.innerHTML = sorted.map(({r,idx}) => {
    ...
    const cardStyle = done
      ...
      : '';
    const doneOverlay = done
      ? `<div style="position:absolute;top:6px;right:6px;background:#48BB78;color:white;font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;">✓ Done</div>`
      : '';

    return `
    <div class="list-item" style="position:relative;${cardStyle}border-left-color:${tierColor};">
      ${doneOverlay}
      <img class="list-item-sprite" src="${sprite(pokemon)}" onerror="handleSpriteError(this,'${pokemon}')">
      <div class="list-item-content">
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
          <span style="font-weight:700;font-size:13px;">${pokemon}</span>
          <span style="background:${tierColor};color:white;font-size:10px;padding:1px 6px;border-radius:10px;font-weight:700;">${tier}</span>
          ${reasons.map(r=>`<span style="font-size:11px;color:#4a5568;">${r}</span>`).join('')}
        </div>
        ${notes ? `<div style="font-size:11px;color:#718096;margin-top:1px;">${notes}</div>` : ''}
      </div>
      <div class="list-item-actions">
        ${!done
          ? `<button class="btn-icon" onclick="confirmRaidDone(${idx})" title="Mark done">✅</button>`
          : `<button class="btn-icon" onclick="undoRaidDone(${idx})" title="Undo">↩️</button>`}
        <button class="btn-icon" onclick="editRaidBoss(${idx})" title="Edit">✏️</button>
        <button class="btn-icon" onclick="deleteRaidBoss(${idx})" title="Delete">🗑️</button>
      </div>
    </div>`;
  }).join('');
  updateRaidWeekLabel();
