...
async function getPokemonData(name) {
  ...
  try {
    ...
    let spriteDefault = apiData.sprites.front_default;
    let spriteShiny   = apiData.sprites.front_shiny;
    const homeDefault = apiData.sprites.other?.home?.front_default || null;

    // For form variants (e.g. furfrou-kabuki), /pokemon/ returns null front_default.
    // Fall back to /pokemon-form/ which has form_sprites populated.
    if (!spriteDefault && !homeDefault) {
      try {
        const formResp = await fetch(`https://pokeapi.co/api/v2/pokemon-form/${cleanName}`);
        if (formResp.ok) {
          const formData = await formResp.json();
          spriteDefault = formData.sprites?.front_default || null;
          spriteShiny   = formData.sprites?.front_shiny   || spriteShiny;
        }
      } catch(_) {}
    }
...
// Trades
const WANTS_HEADERS  = ['Pokemon','Reason','Priority','Notes','Shiny','Legacy','DMAX','GMAX','Background','Costume','Legendary','Purified','New','Form','Quantity'];
const MIRROR_HEADERS = ['Pokemon','Types','Shiny','Legacy','Source','Notes','Priority','Background','Costume','DMAX','GMAX','Legendary','Purified','Form','Quantity'];
...
function renderLuckyWants() {
  ...
  const manual = [...luckyWantsData]
    .map((r, origIdx) => {
      ...
      const form  = r[13] || '';
      const displayName = form ? `${r[0]} (${form})` : r[0];
      return {
        pokemon: r[0], displayName, form,
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${all.map(m => {
      ...
      const frameBorder = m.background
        ? `border:2px solid #C9A227;box-shadow:0 0 0 1px #F0D060 inset,0 2px 10px rgba(201,162,39,0.3);`
        : `border:1.5px solid ${cardBorder};box-shadow:0 2px 8px rgba(0,0,0,0.08);`;
      ...
      <div class="${m.shiny?'card-shiny':''}" style="background:${cardBg};border-radius:10px;padding:6px 6px 5px;width:90px;
        ...
        <img src="${sprite(m.pokemon, null, m.form||null, m.shiny)}" onerror="handleSpriteError(this,'${m.pokemon}')"
...
function renderMirrorTargets() {
  ...
  // Schema: [Pokemon,Types,Shiny,Legacy,Source,Notes,Priority,Background,Costume,DMAX,GMAX,Legendary,Purified,Form]
  const manual = mirrorTargetsData.map((r,idx) => {
    const form = r[13] || '';
    return {
      pokemon:    r[0],
      displayName: form ? `${r[0]} (${form})` : r[0],
      form,
      types:      r[1],
      shiny:      isTrue(r[2]),
      legacy:     isTrue(r[3]),
...
function showAddLuckyEntryModal(editType, editIdx) {
  ...
  if (isEditing) {
    if (editType === 'want') {
      ...
      document.getElementById('leForm').value    = r[13] || '';
      ...
      document.getElementById('leWQuantity').value    = r[14] || '1';
      ...
      document.getElementById('leMQuantity').value    = r[14] || '1';
      ...
      [['leOTagShiny','Shiny'],['leOTagElite','EliteMove'],['leOTagBackground','Background'],
       ...
       ['leOTagDmax','DMAX'],['leOTagGmax','GMAX']].forEach(([id, key]) => {
...
async function saveLuckyEntry(e) {
  ...
  if (type === 'want') {
    const row = [
      ...
      form,  // col 13 — Form
      document.getElementById('leWQuantity').value || '1',  // col 14 — Quantity
    ];
    ...
    const row = [
      ...
      form,  // col 13 — Form
      document.getElementById('leMQuantity').value || '1',  // col 14 — Quantity
    ];
    ...
    const offerTagDefs = [
      ...
      ['leOTagEvent','Event'],['leOTagLegendary','Legendary'],['leOTagRareForm','RareForm'],
      ['leOTagCostume','Costume'],['leOTagPurified','Purified'],
      ['leOTagDmax','DMAX'],['leOTagGmax','GMAX'],
    ];
...
function renderTradeOffers() {
  ...
  const cards = tradeOffersData.map((r, idx) => {
    ...
    const hasBackground = tags.includes('Background');
    const frameBorder = hasBackground
      ? `border:2px solid #C9A227;box-shadow:0 0 0 1px #F0D060 inset,0 2px 10px rgba(201,162,39,0.3);`
      : `border:1px solid #e2e8f0;`;
    const shineBg = isShiny
      ? `background:linear-gradient(135deg,rgba(255,236,153,0.32) 0%,rgba(255,255,255,0) 55%,rgba(255,215,100,0.18) 100%);`
      : `background:white;`;

    return `
    <div style="${shineBg}${frameBorder}border-radius:10px;padding:10px 10px 8px;
                display:flex;flex-direction:column;gap:6px;
                ${isShiny && !hasBackground ? 'box-shadow:0 0 0 2px rgba(214,158,46,0.4);' : ''}">
      <div style="display:flex;align-items:center;gap:8px;">
        ...
        <div style="display:flex;gap:3px;flex-shrink:0;">
          <button class="btn-icon" style="font-size:11px;" onclick="showAddLuckyEntryModal('offer',${idx})" title="Edit">✏️</button>
...
window.onload = () => {
  ...
  // v6 migration: re-fetch all form-variant sprites now that we use /pokemon-form/ fallback
  if (!localStorage.getItem('sprite_cache_v6')) {
    Object.keys(localStorage)
      .filter(k => k.startsWith('pokemon_data_v3_') && k.replace('pokemon_data_v3_','').includes('-'))
      .forEach(k => localStorage.removeItem(k));
    localStorage.setItem('sprite_cache_v6', '1');
  }
