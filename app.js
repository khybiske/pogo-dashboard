...
function renderCandy() {
  ...
  const candidates = powerUpData.map((r, puIdx) => ({r, puIdx})).filter(({r}) => {
  ...
  candidates.sort((a,b) => (PRIO[a.r[12]]||2) - (PRIO[b.r[12]]||2));
  ...
  for (const {r, puIdx} of candidates) {
    const pokemon    = r[0];
    ...
    <div class="list-item${isShiny?' card-shiny':''}${isShadow?' card-shadow':''}" draggable="true" data-alloc-pokemon="${pokemon}"
      ...
      <div style="flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:3px;">
        <button class="btn-icon" style="font-size:11px;margin-bottom:2px;" onclick="showEditPowerUpModal(${puIdx})" title="Edit">✏️</button>
