...
async function loadAllData() {
  ...
  const [pu, candy, wants, mirrors, friends, shinies, events, pvpRoster, pvpConfig, pvpMeta] = await Promise.all([
    ...
    safe('PVPRoster'), safe('PVPConfig'), safe('PVPMeta')
  ]);
...
function renderAll() {
  for (const [fn, name] of [[renderEvents,'Events'],[renderPowerUp,'PowerUp'],[renderCandy,'Candy'],[renderTrades,'Trades'],[renderShinyGaps,'Shinies'],[renderPVP,'PVP']]) {
...
// ── PVP Roster ────────────────────────────────────────────────────────────────
const PVP_HEADERS   = ['Pokemon','PVPpokeRank','IVRank','Ready','Notes'];
...
function renderPVP() {
  ...
  // Build lookup: roster pokemon names (lowercase) for fast cross-reference
  const rosterNames = new Set(pvpRosterData.map(r => (r[0]||'').toLowerCase().trim()));

  // Meta snapshot — compact grid of ranked slots
    ...
    if (!pvpMetaData.length) {
      ...
      metaEl.innerHTML = `
        <div style="font-size:11px;color:#718096;margin-bottom:8px;">
          You have <strong style="color:#48BB78;">${haveCount}</strong> of the top ${metaSorted.length} ·
          <span style="color:#999;">tap a missing one to add it to your roster</span>
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;">
          ${metaSorted.map(r => {
            ...
            // Missing ones are tappable — have ones are not (already in roster)
            const clickable = !have;
            return `
            <div id="metacard-${rank}" onclick="${clickable ? `pvpMetaCardTap(${rank},'${pokemon.replace(/'/g,"\\'")}',this)` : ''}"
                 style="position:relative;display:flex;flex-direction:column;align-items:center;width:58px;
                        opacity:${have ? 1 : 0.6};
                        cursor:${clickable ? 'pointer' : 'default'};
                        transition:opacity 0.15s;">
              <div style="font-size:9px;font-weight:700;color:${have?'#553c9a':'#888'};margin-bottom:2px;">#${rank}</div>
              <img src="${sprite(pokemon)}" onerror="handleSpriteError(this,'${pokemon}')"
                   style="width:40px;height:40px;
                          background:${have?'rgba(85,60,154,0.12)':'rgba(0,0,0,0.06)'};
                          border-radius:6px;padding:2px;
                          border:1px solid ${have?'#b794f4':'#cbd5e0'};
                          ${!have?'filter:grayscale(0.4)':''}">
              <div style="font-size:9px;text-align:center;color:${have?'#2d3748':'#4a5568'};
                          margin-top:2px;line-height:1.2;max-width:58px;overflow:hidden;
                          text-overflow:ellipsis;white-space:nowrap;" title="${pokemon}">${pokemon}</div>
              <div style="font-size:9px;color:${have?'#48BB78':'#a0aec0'};">${have?'✓':'＋'}</div>
            </div>`;
          }).join('')}
        </div>`;
    }
  ...
  if (!pvpRosterData.length) {
    c.innerHTML = '<div class="empty-state">📝 No PVP Pokémon yet — add your GL roster!</div>';
    return;
  }

  // Build meta rank lookup
  const metaRankMap = {};
  pvpMetaData.forEach(r => {
    if (r[1]) metaRankMap[r[1].toLowerCase().trim()] = parseInt(r[0]) || null;
  });

  // Sort roster by PVPpoke rank ascending
  const sorted = pvpRosterData
    .map((r, idx) => ({r, idx}))
    .sort((a, b) => (parseInt(a.r[1])||9999) - (parseInt(b.r[1])||9999));

  // Render cards — type background fetched async and patched in after render
  c.innerHTML = sorted.map(({r, idx}) => {
    ...
    // Ready styling overrides type bg on left border; type bg on the card body
    const borderColor = ready ? '#48BB78' : '#FC8181';
    const readyLabel  = ready
      ? '<span style="background:#C6F6D5;color:#276749;font-size:10px;padding:1px 6px;border-radius:4px;font-weight:600;">✅ Ready</span>'
      : '<span style="background:#FED7D7;color:#C53030;font-size:10px;padding:1px 6px;border-radius:4px;font-weight:600;">⚔️ Not ready</span>';
    const pvpRankBadge = pvpRank
      ? `<span style="background:#553c9a;color:white;font-size:11px;padding:2px 7px;border-radius:10px;font-weight:700;">#${pvpRank}</span>`
      : '';
    const ivRankBadge = ivRank
      ? `<span style="background:#2b6cb0;color:white;font-size:11px;padding:2px 7px;border-radius:10px;font-weight:600;">${ivRank}</span>`
      : '';
    const metaBadge = metaRank
      ? `<span style="background:#d53f8c;color:white;font-size:11px;padding:2px 7px;border-radius:10px;font-weight:700;">Meta #${metaRank}</span>`
      : '';

    return `
    <div id="pvp-card-${idx}" class="list-item" style="border-left-color:${borderColor};align-items:center;">
      <img class="list-item-sprite" src="${sprite(pokemon)}" onerror="handleSpriteError(this,'${pokemon}')">
      <div class="list-item-content">
        <div class="list-item-line1" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
          <span style="font-weight:700;">${pokemon}</span>
          ${metaBadge}${pvpRankBadge}${ivRankBadge}${readyLabel}
        </div>
        ${notes ? `<div class="list-item-line2">${notes}</div>` : ''}
      </div>
      <div class="list-item-actions">
  ...
  // Async patch type gradients onto cards
  sorted.forEach(({r, idx}) => {
    const pokemon = r[0] || '';
    if (!pokemon) return;
    const clean = pokemon.toLowerCase().replace(/^(alolan|galarian|hisuian|shadow|mega|primal) /,'').replace(/\s+/g,'-');
    getPokemonData(clean).then(data => {
      if (!data || !data.types) return;
      const bg = getTypeBackground(data.types);
      const card = document.getElementById(`pvp-card-${idx}`);
