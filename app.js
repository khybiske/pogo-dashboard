...
function sprite(name, regional, form, shiny) {
  ...
  // Immediate fallback — Pokémon Showdown has shiny sprites too
  if (shiny) return `https://play.pokemonshowdown.com/sprites/gen5-shiny/${safeName}.png`;
...
function renderLuckyWants() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${sorted.map(({r,origIdx}) => {
      ...
      return `
      <div style="background:${cardBg};border-radius:10px;padding:7px 7px 6px;width:100px;
                  box-shadow:0 2px 8px rgba(0,0,0,0.08);border:1.5px solid ${cardBorder};
                  display:flex;flex-direction:column;align-items:center;text-align:center;min-height:110px;">
        <img src="${sprite(pokemon)}" onerror="handleSpriteError(this,'${pokemon}')"
             style="width:48px;height:48px;image-rendering:pixelated;">
        <div style="font-size:11px;font-weight:700;color:#2d3748;margin-top:3px;line-height:1.2;">${pokemon}</div>
        ${badges ? `<div style="font-size:12px;margin-top:1px;">${badges}</div>` : ''}
    ...
    ${allMirrors.map(m => {
      ...
      <div style="background:${cardBg};border-radius:10px;padding:7px 7px 6px;width:100px;
        ...
        <img src="${sprite(m.pokemon, null, null, m.shiny)}" onerror="handleSpriteError(this,'${m.pokemon}')"
             style="width:48px;height:48px;image-rendering:pixelated;">
        <div style="font-size:11px;font-weight:700;color:#2d3748;margin-top:3px;line-height:1.2;">${m.displayName}</div>
        ${badgeIcons ? `<div style="font-size:12px;margin-top:1px;">${badgeIcons}</div>` : ''}
        ${sourceLabel}
...
window.onload = () => {
  ...
  if (!localStorage.getItem('sprite_cache_v2')) {
    ...
    localStorage.setItem('sprite_cache_v2', '1');
  }
