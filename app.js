...
const CHECKLIST_HEADERS = ['EventName','Item','Day','Done'];

function renderEventChecklist() {
  const c = document.getElementById('eventChecklistContent');
  if (!c) return;

  if (!eventChecklistData.length) {
    c.innerHTML = `<div class="empty-state">No checklist items yet — hit ➕ Add Item to get started.</div>`;
    return;
  }
  ...
  eventChecklistData.forEach((r, idx) => {
    ...
    if (!groups[eventName]) { groups[eventName] = []; groupOrder.push(eventName); }
    groups[eventName].push({ r, idx });
  });

  const totalItems = eventChecklistData.length;
  const doneItems  = eventChecklistData.filter(r => isTrue(r[3])).length;

  c.innerHTML = groupOrder.map(eventName => {
    const items = groups[eventName];

    // Sub-group by Day within the event
    const dayGroups = {};
    const dayOrder  = [];
    items.forEach(({ r, idx }) => {
      const day = (r[2] || '').trim() || '';
      const key = day || '__all__';
    ...
    const progressBar = `
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;">
        <div style="flex:1;height:6px;background:#e2e8f0;border-radius:3px;overflow:hidden;">
          <div style="width:${pct}%;height:100%;background:${pct===100?'#48BB78':'#667eea'};border-radius:3px;transition:width 0.3s;"></div>
        </div>
        <span style="font-size:10px;color:#718096;white-space:nowrap;">${doneCount}/${totalCount}</span>
      </div>`;
    ...
    const dayHtml = dayOrder.map(key => {
      ...
      const dayHeader = dayLabel
        ? `<div style="font-size:9px;font-weight:700;color:#667eea;text-transform:uppercase;letter-spacing:0.5px;margin:6px 0 3px;">${dayLabel}</div>`
        : '';

      const itemRows = dayItems.map(({ r, idx }) => {
        const done = isTrue(r[3]);
        const item = r[1] || '';
        return `
          <div style="display:flex;align-items:center;gap:8px;padding:5px 6px;border-radius:6px;
                      background:${done?'rgba(72,187,120,0.06)':'white'};
                      border:1px solid ${done?'#c6f6d5':'#e2e8f0'};margin-bottom:3px;cursor:pointer;"
               onclick="toggleChecklistItem(${idx})">
            <span style="font-size:16px;flex-shrink:0;">${done ? '✅' : '⬜'}</span>
            <span style="flex:1;font-size:12px;color:${done?'#a0aec0':'#2d3748'};
                         text-decoration:${done?'line-through':'none'};line-height:1.3;">${item}</span>
            <div style="display:flex;gap:4px;flex-shrink:0;">
              <button class="btn-icon" style="font-size:10px;" onclick="event.stopPropagation();editChecklistItem(${idx})" title="Edit">✏️</button>
              <button class="btn-icon" style="font-size:10px;" onclick="event.stopPropagation();deleteChecklistItem(${idx})" title="Delete">🗑️</button>
            </div>
          </div>`;
      }).join('');

      return dayHeader + itemRows;
    }).join('');

    return `
      <div style="margin-bottom:12px;">
        <div style="font-size:12px;font-weight:700;color:#2d3748;margin-bottom:5px;display:flex;align-items:center;gap:6px;">
          <span>🎪 ${eventName}</span>
          ${pct === 100 ? '<span style="font-size:10px;color:#48BB78;font-weight:600;">All done! 🎉</span>' : ''}
        </div>
        ${progressBar}
        ${dayHtml}
      </div>`;
  }).join('') + (totalItems > 1 ? `<div style="font-size:10px;color:#a0aec0;text-align:right;margin-top:4px;">${doneItems}/${totalItems} total complete</div>` : '');
}
...
async function toggleChecklistItem(idx) {
  ...
  setBgSaving(true);
  await saveSheetData('EventChecklist', [CHECKLIST_HEADERS, ...eventChecklistData]);
  setBgSaving(false);
}
...
function showAddChecklistItemModal() {
  ...
  // Populate datalist with existing event names
...
async function saveChecklistItem(e) {
  ...
  const row = [
    document.getElementById('clEventName').value.trim(),
    document.getElementById('clItem').value.trim(),
    document.getElementById('clDay').value.trim(),
    idx !== '' ? (eventChecklistData[idx]?.[3] || 'FALSE') : 'FALSE', // preserve done state on edit
  ];
  if (idx !== '') { eventChecklistData[idx] = row; } else { eventChecklistData.push(row); }
  const ok = await saveSheetData('EventChecklist', [CHECKLIST_HEADERS, ...eventChecklistData]);
  setSaving(btn, false);
...
async function clearAllChecklistItems() {
  if (!eventChecklistData.length) return;
  if (!confirm('Reset ALL checklist items (uncheck everything)?')) return;
  setBgSaving(true);
  eventChecklistData.forEach(r => r[3] = 'FALSE');
  await saveSheetData('EventChecklist', [CHECKLIST_HEADERS, ...eventChecklistData]);
  setBgSaving(false);
  renderEventChecklist();
}
