...
async function loadAllData() {
  ...
  // Fetch shinyrates.com in parallel — routed via Apps Script to avoid CORS
  ...
  const fetchShinyRates = async () => {
    try {
      ...
      console.warn('shinyrates fetch failed:', e);
      const cached = cacheRead('__shinyrates__');
  ...
  const renderCard = ({ name, denom, total, id }, isUnrated = false) => {
    ...
    if (isUnrated || denom === null) {
      ...
      <div style="display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 6px;
                  border-radius:8px;background:rgba(0,0,0,0.02);border:1px solid #e2e8f0;text-align:center;opacity:0.7;">
        <img src="${spriteUrl}" style="width:52px;height:52px;" ${fallbackUrl ? `onerror="this.src='${fallbackUrl}'"` : `onerror="handleSpriteError(this,'${name}')"`}>
        <div style="font-size:11px;font-weight:700;color:#2d3748;line-height:1.2;">${name}</div>
        <div style="font-size:9px;color:#999;">No rate data</div>
      </div>`;
    }
    ...
    return `
    <div style="display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 6px;
                border-radius:8px;background:${t.bg};border:1px solid ${t.color}22;text-align:center;">
      <img src="${spriteUrl}" style="width:52px;height:52px;" ${fallbackUrl ? `onerror="this.src='${fallbackUrl}'"` : `onerror="handleSpriteError(this,'${name}')"`}>
      <div style="font-size:11px;font-weight:700;color:#2d3748;line-height:1.2;">${name}</div>
      <div style="font-size:10px;font-weight:600;color:${t.color};">1/${denom.toLocaleString()}</div>
      <div style="font-size:9px;padding:1px 5px;border-radius:4px;background:${t.color};color:white;font-weight:700;">${t.label}</div>
      ${lowSample ? `<div style="font-size:9px;color:#C05621;" title="Low sample — rate may not be reliable">⚠️ n=${total}</div>` : ''}
    </div>`;
  };

  const grid = `display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:8px;`;
