...
// Trades
const WANTS_HEADERS  = ['Pokemon','Reason','Priority','Notes','Shiny','Legacy','DMAX','GMAX','Background','Costume','Legendary','Purified','New','Form'];
const MIRROR_HEADERS = ['Pokemon','Types','Shiny','Legacy','Source','Notes','Priority','Background','Costume','DMAX','GMAX','Legendary','Purified','Form'];
...
window.onload = () => {
  ...
  // v5 migration: clear ALL form-variant sprite cache entries (containing a hyphen after base name)
  // so they get freshly fetched with the corrected slug logic.
  if (!localStorage.getItem('sprite_cache_v5')) {
    Object.keys(localStorage)
      .filter(k => k.startsWith('pokemon_data_v3_') && k.replace('pokemon_data_v3_','').includes('-'))
      .forEach(k => localStorage.removeItem(k));
    localStorage.setItem('sprite_cache_v5', '1');
  }
