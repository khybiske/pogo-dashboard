...
function renderCandy() {
  ...
  for (const r of candidates) {
    ...
    // Build display name matching power-up section
    let displayName = pokemon;
    if (regional) displayName = `${regional} ${pokemon}`;
    if (form)     displayName = `${displayName} (${form})`;
    ...
    const speciesXLNote = speciesXL > 0 && needXLRaw > 0
      ? `<div style="font-size:9px;color:#48BB78;margin-top:1px;">+${speciesXL} species XL owned</div>` : '';

    const needStr = [
      needRC > 0 ? `${needRC}${RC}` : '',
      needXLRaw > 0 ? `${needXL}${RCX}${speciesXL>0?`<span style="font-size:9px;color:#48BB78;"> (${speciesXL} owned)</span>`:''}` : '',
    ...
    html += `
    <div class="list-item" draggable="true" data-alloc-pokemon="${pokemon}"
         style="display:grid;grid-template-columns:auto auto 1fr auto auto;gap:0;align-items:center;
                padding:6px 8px;${rowBg}margin-bottom:3px;">
      <span class="drag-handle" style="margin-right:6px;">☰</span>
      <img src="${sprite(pokemon, regional, form)}" onerror="handleSpriteError(this,'${pokemon}')"
      ...
      <div style="font-size:12px;font-weight:600;min-width:0;">
        ...
        <span style="font-size:10px;color:#999;font-weight:400;margin-left:4px;">Lv${curLv}→50</span>
      </div>
      <div style="font-size:11px;text-align:right;color:#4a5568;padding:0 10px;line-height:1.6;white-space:nowrap;">${needStr}</div>
      <div style="text-align:center;white-space:nowrap;">
        ${needRC > 0 ? `<div style="display:flex;align-items:center;gap:2px;justify-content:center;margin-bottom:2px;">
          ...
          <input type="number" value="${allocRC}" min="0"
            style="width:40px;text-align:center;font-size:11px;border:1px solid #e2e8f0;border-radius:4px;padding:2px 2px;"
            onchange="rcAllocSet('${pokemon}',false,this.value)">${RC}
          <button class="btn-icon" style="font-size:10px;padding:1px 4px;" onclick="rcAllocAdj('${pokemon}',false,10)">+</button>
          <span style="font-size:10px;color:${stillRC>0?'#E53E3E':'#48BB78'};margin-left:4px;min-width:30px;">${stillRC}${RC}</span>
        </div>` : ''}
        ${needXL > 0 ? `<div style="display:flex;align-items:center;gap:2px;justify-content:center;">
          ...
          <button class="btn-icon" style="font-size:10px;padding:1px 4px;" onclick="rcAllocAdj('${pokemon}',true,10)">+</button>
          <span style="font-size:10px;color:${stillXL>0?'#E53E3E':'#48BB78'};margin-left:4px;min-width:30px;">${stillXL}${RCX}</span>
        </div>` : ''}
      </div>
    </div>`;
  }
...
function renderLuckyWants() {
  ...
  const prioColors = {High:'#E53E3E',Medium:'#D69E2E',Low:'#3182CE'};
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${sorted.map(({r,origIdx}) => {
      ...
      const prioColor = prioColors[prio] || '#718096';
      const badges = [shiny?'✨':'', legacy?'£':''].filter(Boolean).join(' ');

      return `
      <div style="background:white;border-radius:10px;padding:10px 10px 8px;width:110px;
                  box-shadow:0 2px 8px rgba(0,0,0,0.1);border-top:3px solid ${prioColor};
        ...
        <div style="font-size:11px;font-weight:700;color:#2d3748;margin-top:4px;line-height:1.2;">${pokemon}</div>
        ${badges ? `<div style="font-size:12px;margin-top:2px;">${badges}</div>` : ''}
        <span style="background:${prioColor};color:white;font-size:9px;font-weight:700;
                     padding:1px 6px;border-radius:8px;margin-top:3px;">${prio}</span>
        ${reason ? `<div style="font-size:9px;color:#718096;margin-top:3px;line-height:1.2;">${reason}</div>` : ''}
        ${notes  ? `<div style="font-size:9px;color:#999;margin-top:2px;line-height:1.2;font-style:italic;">${notes}</div>` : ''}
        <div style="display:flex;gap:4px;margin-top:6px;">
...
function renderMirrorTargets() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${allMirrors.map(m => {
      const accentColor = m.isAuto ? '#D69E2E' : '#48BB78';
      const badgeIcons  = [m.shiny?'✨':'', m.legacy?'£':'', m.dmax?'💎':'', m.gmax?'🌟':''].filter(Boolean).join(' ');
      const prioColors  = {High:'#E53E3E', Medium:'#D69E2E', Low:'#3182CE'};
      const prioColor   = prioColors[m.priority] || '#718096';

      return `
      <div style="background:white;border-radius:10px;padding:10px 10px 8px;width:120px;
                  box-shadow:0 2px 8px rgba(0,0,0,0.1);border-top:3px solid ${accentColor};
                  display:flex;flex-direction:column;align-items:center;text-align:center;">
        <img src="${sprite(m.pokemon)}" onerror="handleSpriteError(this,'${m.pokemon}')"
             style="width:52px;height:52px;image-rendering:pixelated;">
        <div style="font-size:11px;font-weight:700;color:#2d3748;margin-top:4px;line-height:1.2;">${m.displayName}</div>
        ${badgeIcons ? `<div style="font-size:12px;margin-top:2px;">${badgeIcons}</div>` : ''}
        <span style="background:${prioColor};color:white;font-size:9px;font-weight:700;
                     padding:1px 6px;border-radius:8px;margin-top:3px;">${m.priority}</span>
        <div style="font-size:9px;color:${accentColor};font-weight:600;margin-top:3px;">
          ${m.isAuto ? '⚡ Auto-synced' : '✋ Manual'}
        </div>
        ${m.notes ? `<div style="font-size:9px;color:#999;margin-top:2px;line-height:1.2;font-style:italic;">${m.notes}</div>` : ''}
        ${!m.isAuto ? `
        <div style="display:flex;gap:4px;margin-top:6px;">
