...
function renderWhoMax() {
  ...
  c.innerHTML = shown.map(({ r, pokemon, curLv, speciesXL, speciesCandy, regional, form,
    ...
    if (score === 0) {
      readyLabel = '✅ Ready to max!'; readyColor = '#48BB78';
    } else if (readiness >= 0.6) {
      readyLabel = '🔥 Almost there';  readyColor = '#C05621';
    } else if (readiness >= 0.3) {
      readyLabel = '⚙️ In progress';   readyColor = '#D69E2E';
    } else {
      readyLabel = '📋 Needs work';    readyColor = '#718096';
    }
    ...
    // Candy rows — regular
    const regularCoveredPct = needRCraw > 0 ? Math.min(100, Math.round(speciesCandy / needRCraw * 100)) : 100;
    ...
    let cardBg = 'white';
    if (isShiny && isShadow) cardBg = 'linear-gradient(135deg,rgba(107,70,193,0.08) 0%,white 50%,rgba(255,215,0,0.07) 100%)';
    else if (isShiny)        cardBg = 'linear-gradient(135deg,rgba(255,215,0,0.08) 0%,white 60%)';
    else if (isShadow)       cardBg = 'linear-gradient(135deg,rgba(107,70,193,0.14) 0%,rgba(128,90,213,0.06) 50%,white 100%)';
    ...
    <div style="display:grid;grid-template-columns:52px 1fr;gap:10px;align-items:start;
      ...
      <div style="min-width:0;">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:6px;margin-bottom:4px;flex-wrap:wrap;">
          ...
          <span style="font-size:10px;color:#999;">Lv${curLv} → 50</span>
        </div>
