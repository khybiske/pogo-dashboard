...
function renderWhoMax() {
  ...
  c.innerHTML = shown.map(({ r, pokemon, curLv, speciesXL, speciesCandy, regional, form,
    ...
    else if (isShiny)        cardBg = `linear-gradient(135deg,rgba(255,215,0,0.10) 0%,${statusTint} 60%)`;
...
function renderLuckyWants() {
  ...
  c.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${sorted.map(({r,origIdx}) => {
      ...
      const dmax       = isTrue(r[6]);
      const gmax       = isTrue(r[7]);
      const background = isTrue(r[8]);
      const costume    = isTrue(r[9]);
      const legendary  = isTrue(r[10]);
      const purified   = isTrue(r[11]);
      const isNew      = isTrue(r[12]);
      const special    = isSpecialTrade({ shiny, legendary, costume, purified, gmax, background, isNew });
      ...
      const badges = [
        shiny?'✨':'', legacy?'£':'', dmax?'💎':'', gmax?'🌟':'',
        background?'🖼️':'', costume?'🎭':'', legendary?'🏆':'', purified?'🤍':'', isNew?'🆕':'',
      ].filter(Boolean).join(' ');
      ...
      <div class="${shiny?'card-shiny':''}" style="background:${cardBg};border-radius:10px;padding:6px 6px 5px;width:90px;
        ...
        ${reason ? `<div class="lw-notes" style="font-size:9px;color:#718096;margin-top:2px;line-height:1.2;">${reason}</div>` : ''}
        ${notes  ? `<div class="lw-notes" style="font-size:9px;color:#999;margin-top:1px;line-height:1.2;font-style:italic;">${notes}</div>` : ''}
...
function renderMirrorTargets() {
  ...
  const autoSynced = powerUpData
    ...
    .map(r => {
      ...
      return {
        ...
        background: false,
        costume: false,
        legendary: false, // PowerUp entries don't track legendary; can be flagged manually
        source: 'PowerUp',
  ...
  // Schema: [Pokemon,Types,Shiny,Legacy,Source,Notes,Priority,Background,Costume,DMAX,GMAX,Legendary]
...
function renderLuckyFriends() {
  ...
  const isMobile = window.innerWidth <= 600;
  const displayList = (simpleModeActive && isMobile) ? active.slice(0, 5) : active;

  c.innerHTML = displayList.map(({r, idx}) => {
...
function renderTradeOffers() {
  ...
  const cards = tradeOffersData.map((r, idx) => {
    ...
    <div style="background:white;border:1px solid #e2e8f0;border-radius:10px;padding:10px 10px 8px;
                ...
                ${isShiny ? 'box-shadow:0 0 0 2px rgba(214,158,46,0.4);background:linear-gradient(135deg,rgba(255,240,180,0.25) 0%,white 60%);' : ''}">
