// GENERATED: 2026-02-18-2120-CD-SPRITES-COMPLETE
// Added missing CD Pokemon IDs: Poliwag, Wooper, Timburr, Shelmet, Rowlet
...
const FRIENDS_HEADERS = ['Friend','They Want','You Want','Notes','Status','Ready'];
...
function renderMirrorTargets() {
  ...
  const autoSynced = powerUpData
    ...
    .map(r => ({
      ...
      isAuto: true,
      priority: r[12] || 'Medium' // Get priority from Power-Up
    }));
  ...
  const manual = mirrorTargetsData.map((r,idx) => ({
    ...
    isAuto: false,
    idx: idx,
    priority: r[6] || 'Medium' // Get priority from sheet
  }));
  ...
  c.innerHTML = allMirrors.map(m => {
    ...
    <div class="list-item" style="border-left-color:${m.isAuto?'#FFD700':'#48BB78'};cursor:default;">
      <img class="list-item-sprite" src="${sprite(m.pokemon)}" onerror="this.src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/25.png'">
      <div class="list-item-content">
        <div class="list-item-line1">${m.pokemon} ${badges.join('')}</div>
        <div class="list-item-line2">
...
function renderLuckyFriends() {
  ...
  c.innerHTML = luckyFriendsData.map((r,idx)=>{
    ...
    // Color code: Ready=green, Planned=yellow, Waiting/other=default
    let nameColor = '#ccc'; // default
    if (isReady) {
      nameColor = '#48BB78'; // green
    } else if (status === 'planned') {
      nameColor = '#FFA500'; // yellow/orange
    }
    ...
    <div class="list-item" draggable="true" data-index="${idx}">
      ...
      <div class="list-item-content">
        <div class="list-item-line1" style="color:${nameColor};font-weight:600;">
          ${r[0]}
        </div>
        <div class="list-item-line2" style="display:grid;grid-template-columns:auto 1fr;gap:8px;align-items:start;">
          <span style="color:#999;">They want:</span><span>${r[1]||'TBD'}</span>
          <span style="color:#999;">You want:</span><span>${r[2]||'TBD'}</span>
          ${r[3]?`<span style="color:#999;">Notes:</span><span>${r[3]}</span>`:''}
        </div>
      </div>
      <div class="list-item-actions">
...
function renderShinyGaps() {
  ...
  c.innerHTML = '<div class="shiny-grid">'+sorted.map(({r,idx})=>{
    ...
    <div class="${cardClass}" onclick="cycleShinyPriority(${idx})" style="cursor:pointer;" title="Click to cycle: Want → High Priority → Got it!">
      <img src="${sprite(r[0])}" class="shiny-sprite" onerror="this.src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/25.png'" draggable="false">
...
async function cycleShinyPriority(idx) {
  console.log(`Clicked shiny card index: ${idx}`); // Debug log
  ...
  console.log(`Current state: Have=${currentHave}, Priority=${currentPriority}`);
  
  // Cycle: Normal → High → Got it (YES)
  if (currentHave === 'YES' || currentHave === 'Y' || currentHave === 'TRUE') {
    row[1] = 'NO';
    row[2] = 'Normal';
    console.log('Reset to: Want (Normal)');
  } else if (currentPriority === 'HIGH') {
    row[1] = 'YES';
    row[2] = 'Normal';
    console.log('Marked as: Got it! (will disappear)');
  } else {
    row[2] = 'High';
    console.log('Changed to: High Priority');
  }
