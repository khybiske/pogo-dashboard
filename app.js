...
function parseRate(rateStr) {
  ...
  const num = parseInt(rateStr.replace(/[^0-9]/g, ''));
...
function renderMissingRareShinies() {
  ...
  const tiers = [
    { min: 1000, label: 'Ultra Rare', color: '#553c9a', bg: 'rgba(85,60,154,0.08)'  },
    { min: 600,  label: 'Very Rare',  color: '#2b6cb0', bg: 'rgba(43,108,176,0.08)' },
    { min: 300,  label: 'Rare',       color: '#276749', bg: 'rgba(39,103,73,0.08)'  },
    { min: 0,    label: 'Common',     color: '#718096', bg: 'rgba(113,128,150,0.06)' },
  ];
  ...
  const renderCard = ({ name, denom, total, id }, isUnrated = false) => {
    ...
    return `
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;padding:6px 4px;
                border-radius:8px;border:1px solid ${t.color}22;${shinyGrad};text-align:center;
                box-shadow:inset 0 0 0 1px ${t.color}18;">
      <img src="${spriteUrl}" style="width:40px;height:40px;" ${fallbackUrl ? `onerror="this.src='${fallbackUrl}'"` : `onerror="handleSpriteError(this,'${name}')"`}>
      <div style="font-size:10px;font-weight:700;color:#2d3748;line-height:1.2;">${name}</div>
      <div style="font-size:9px;font-weight:600;color:${t.color};">1/${denom.toLocaleString()}</div>
      <div style="font-size:8px;padding:1px 4px;border-radius:3px;background:${t.color};color:white;font-weight:700;">${t.label}</div>
