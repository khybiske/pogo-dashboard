...
// Tracks pending "caught" confirmations: rowIdx → timeoutId
...
async function tapShinyCaught(idx) {
  const row = shinyGapsData[idx];
  if (!row) return;

  // If already in grace period → undo
  if (shinyMissingCaught.has(idx)) {
    clearTimeout(shinyMissingCaught.get(idx));
  ...
  // Mark caught, re-render showing overlay
  row[1] = 'TRUE';
  renderMissingRareShinies();
  setBgSaving(true);
  await saveSheetData('ShinyCollection', [['Pokemon','Have','Priority','CD'], ...shinyGapsData]);
  ...
  // Grace period: tap again to undo, or auto-dismiss after 3s
...
function renderMissingRareShinies() {
  ...
  const enriched = displayRows.map(({ r, idx }) => {
    ...
    return { name, dex, denom, total, id: entry?.id || null, idx, isCaught: shinyMissingCaught.has(idx) };
  });
  ...
  const renderCard = ({ name, dex, denom, total, id, idx, isCaught }) => {
    const spriteUrl   = id
    ...
    const borderStyle = isCaught
      ? 'border:2px solid #48BB78;'
      : `border:1px solid ${t ? t.color + '18' : '#e2e8f0'};`;

    const catchOverlay = isCaught ? `
      <div style="position:absolute;inset:0;border-radius:8px;background:rgba(72,187,120,0.88);
        ...
        <div style="font-size:7px;color:rgba(255,255,255,0.85);">Tap to undo</div>
      </div>` : '';
  ...
  const missingCount = enriched.filter(e => !e.isCaught).length;
...
function renderHotShinies() {
  ...
  const cards = hot.map(({ name, denom, total, id, have }) => {
    ...
    return `<div style="display:flex;flex-direction:column;align-items:center;gap:3px;padding:8px 6px;
                         border-radius:8px;background:${heatBg};${borderCss}text-align:center;">
      <img src="${spriteUrl}" style="width:52px;height:52px;" onerror="this.src='${fallback}'">
      <div style="font-size:11px;font-weight:700;color:#2d3748;line-height:1.2;">${name}</div>
      ${detailHtml}
    </div>`;
  }).join('');
