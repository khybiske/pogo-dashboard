...
function applySheetData(name, raw) {
  switch(name) {
    ...
    case 'CommunityDayShinies': shinyGapsData   = raw.length > 1 ? raw.slice(1) : []; break;
...
const ALL_SHEETS = [
  ...
  'CommunityDayShinies','CurrentEvents','PVPRoster','PVPConfig','PVPMeta','RaidBosses',
...
function renderAll() {
  for (const [fn, name] of [[renderEvents,'Events'],[renderRaidBosses,'Raids'],[renderPowerUp,'PowerUp'],[renderWhoMax,'WhoMax'],[renderCandy,'Candy'],[renderTrades,'Trades'],[renderShinyGaps,'Shinies'],[renderMissingRareShinies,'MissingRare'],[renderHotShinies,'HotShinies'],[renderPVP,'PVP'],[renderShinyOdds,'ShinyOdds'],[renderMedals,'Medals'],[renderUpcomingCDs,'UpcomingCDs'],[renderLinks,'Links']]) {
...
function renderMissingRareShinies() {
  ...
  if (!shinyGapsData.length) {
    c.innerHTML = '<div class="empty-state">No shiny data yet — populate your CommunityDayShinies sheet.</div>';
    return;
  }

  const ratesMap = shinyRatesData.length ? buildRatesLookup() : {};

  // All missing shinies (have !== true)
  const missing = shinyGapsData.filter(r => {
    const have = String(r[1] ?? '').toUpperCase();
    return have !== 'TRUE' && have !== 'YES' && have !== 'Y';
  });
  ...
  // Enrich with rate data — separate into rated and unrated
  const rated   = [];
  const unrated = [];
  ...
  missing.forEach(r => {
    ...
    if (entry && !EVOLUTION_IDS.has(entry.id) && entry.denom !== null) {
      rated.push({ name, denom: entry.denom, total: entry.total, id: entry.id });
    } else {
      // Keep unrated/evolution-unknown entries but flag them
      unrated.push({ name, denom: null, total: 0, id: entry?.id || null });
    }
  });

  // Sort rated rarest first
  rated.sort((a, b) => b.denom - a.denom);
  ...
  const renderCard = ({ name, denom, total, id }, isUnrated = false) => {
    const spriteUrl = id
    ...
    // Subtle static shiny gradient — gold shimmer tint
    const shinyGrad = 'background:linear-gradient(135deg,rgba(255,236,153,0.35) 0%,rgba(255,255,255,0) 60%,rgba(255,215,100,0.2) 100%)';

    if (isUnrated || denom === null) {
      return `
      <div style="display:flex;flex-direction:column;align-items:center;gap:2px;padding:6px 4px;
                  border-radius:8px;border:1px solid #e2e8f0;${shinyGrad};text-align:center;opacity:0.65;">
        <img src="${spriteUrl}" style="width:40px;height:40px;" ${fallbackUrl ? `onerror="this.src='${fallbackUrl}'"` : `onerror="handleSpriteError(this,'${name}')"`}>
        <div style="font-size:10px;font-weight:700;color:#2d3748;line-height:1.2;">${name}</div>
        <div style="font-size:8px;color:#999;">No rate data</div>
      </div>`;
    }

    const t = getTier(denom);
    return `
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;padding:6px 4px;
                border-radius:8px;border:1px solid ${t.color}18;${shinyGrad};text-align:center;
                box-shadow:inset 0 0 0 1px ${t.color}12;">
      <img src="${spriteUrl}" style="width:40px;height:40px;" ${fallbackUrl ? `onerror="this.src='${fallbackUrl}'"` : `onerror="handleSpriteError(this,'${name}')"`}>
      <div style="font-size:10px;font-weight:700;color:#2d3748;line-height:1.2;">${name}</div>
      ...
      ${lowSample ? `<div style="font-size:8px;color:#C05621;" title="Low sample — rate may not be reliable">⚠️ n=${total}</div>` : ''}
    </div>`;
  };

  const grid = `display:grid;grid-template-columns:repeat(auto-fill,minmax(85px,1fr));gap:6px;`;

  let html = `<div style="font-size:11px;color:#999;margin-bottom:8px;">
    ${missing.length} missing · ${rated.length} with live rate data · sorted rarest first
  </div>`;

  html += `<div style="${grid}">${rated.map(r => renderCard(r)).join('')}</div>`;

  if (unrated.length) {
    html += `
    <div style="font-size:11px;font-weight:700;color:#999;text-transform:uppercase;letter-spacing:0.5px;
                margin:14px 0 6px;">No rate data available</div>
    <div style="${grid}">${unrated.map(r => renderCard(r, true)).join('')}</div>`;
  }
...
function renderHotShinies() {
  ...
  const HOT_THRESHOLD = 100; // denominator < 100 = meaningfully boosted
  const ratesMap = buildRatesLookup();
  ...
  const hot = shinyRatesData
    ...
    .filter(Boolean)
    .sort((a, b) => a.denom - b.denom); // hottest first
  ...
  c.innerHTML = `
    <div style="font-size:10px;color:#999;margin-bottom:8px;">
      Live data from shinyrates.com · Snapshot at ${now} · Rates update every minute
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:8px;">` +
    hot.map(({ name, denom, total, id, have }) => {
      const lowSample  = total < 200;
      const heatColor  = denom < 20  ? '#C05621'   // extremely hot
                       : denom < 50  ? '#D69E2E'   // very hot
                       :               '#3182CE';   // boosted
      ...
      const haveBadge  = have
        ? `<div style="font-size:9px;color:#48BB78;font-weight:700;">✓ Have it</div>`
        : `<div style="font-size:9px;color:#C05621;font-weight:700;">✗ Missing</div>`;
      return `
      <div style="display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 6px;
                  border-radius:8px;background:rgba(0,0,0,0.02);border:1px solid ${heatColor}44;
                  border-top:3px solid ${heatColor};text-align:center;">
        <img src="${spriteUrl}" style="width:52px;height:52px;" onerror="this.src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png'">
        ...
        ${haveBadge}
        ${lowSample ? `<div style="font-size:9px;color:#718096;" title="Small sample — treat with caution">⚠️ n=${total}</div>` : `<div style="font-size:9px;color:#999;">n=${total.toLocaleString()}</div>`}
      </div>`;
    }).join('') + '</div>';
}
...
function renderShinyGaps() {
  ...
  if (!shinyGapsData.length) { 
    c.innerHTML='<div class="empty-state">Update Google Sheet "CommunityDayShinies" with Pokemon | Have | Priority | CD</div>'; 
    return; 
  }
  
  // CD Gaps section only shows rows where CD column (r[3]) is TRUE — or all rows if no CD column populated yet
  const hasCDColumn = shinyGapsData.some(r => String(r[3] ?? '').toUpperCase() === 'TRUE');
  const cdRows = hasCDColumn
    ? shinyGapsData.map((r,idx)=>({r,idx})).filter(({r}) => String(r[3] ?? '').toUpperCase() === 'TRUE')
    : shinyGapsData.map((r,idx)=>({r,idx})); // graceful fallback if CD column not yet added

  const missing = cdRows
    .filter(({r,idx})=>{
      const v=String(r[1] ?? '').toUpperCase();
  ...
  const sorted = missing.sort((a,b)=>{
    const prioA = String(a.r[2] ?? '').toUpperCase();
    const prioB = String(b.r[2] ?? '').toUpperCase();
  ...
  c.innerHTML = '<div class="shiny-grid">'+sorted.map(({r,idx})=>{
    const priority = String(r[2] ?? 'Normal').toUpperCase();
...
async function cycleShinyPriority(idx) {
  ...
  const currentPriority = String(row[2] ?? 'Normal').toUpperCase();
  const currentHave     = String(row[1] ?? 'NO').toUpperCase();
  ...
  if (shinyPendingGotIt.has(idx)) {
    ...
    renderShinyGaps();
    setBgSaving(true);
    await saveSheetData('CommunityDayShinies', [['Pokemon','Have','Priority','CD'], ...shinyGapsData]);
    setBgSaving(false);
    return;
  }
