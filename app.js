...
const TRADE_OFFER_HEADERS = ['Pokemon','Description','Quantity','Notes','Tags'];

function renderTradeOffers() {
  const c = document.getElementById('tradeOffersContent');
  if (!c) return;
  if (!tradeOffersData.length) {
    c.innerHTML = '<div class="empty-state">💎 Nothing listed yet — add Pokémon worth trading to help plan your offers.</div>';
    return;
  }

  const TAG_COLORS = {
    'shiny':       '#D69E2E', 'elite move':  '#C53030', 'elite':  '#C53030',
    'background':  '#2B6CB0', 'event':       '#553C9A', 'legendary': '#B7791F',
    'costume':     '#276749', 'purified':    '#6B46C1',
  };

  c.innerHTML = tradeOffersData.map((r, idx) => {
    const pokemon = r[0] || '';
    const desc    = r[1] || '';
    const qty     = parseInt(r[2]) || 1;
    const notes   = r[3] || '';
    const tags    = (r[4] || '').split(',').map(t => t.trim()).filter(Boolean);

    const tagPills = tags.map(t => {
      const key   = t.toLowerCase();
      const color = Object.entries(TAG_COLORS).find(([k]) => key.includes(k))?.[1] || '#718096';
      return `<span style="background:${color};color:white;font-size:9px;padding:1px 6px;border-radius:10px;font-weight:700;">${t}</span>`;
    }).join('');

    const spriteSlug = pokemon.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9\-]/g,'');
    const qtyBadge = qty > 1
      ? `<span style="background:#2b6cb0;color:white;font-size:10px;padding:1px 6px;border-radius:10px;font-weight:700;">×${qty}</span>`
      : '';

    return `
    <div class="list-item" style="align-items:center;padding-top:5px;padding-bottom:5px;">
      <img class="list-item-sprite" src="${sprite(spriteSlug)}" onerror="this.style.display='none'"
           style="width:36px;height:36px;">
      <div class="list-item-content" style="gap:2px;">
        ...
        <div style="font-size:11px;color:#4a5568;font-weight:600;">${desc}</div>
        <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:2px;">
          ${tagPills}
          ${notes ? `<span style="font-size:10px;color:#718096;font-style:italic;">· ${notes}</span>` : ''}
        </div>
      </div>
      <div class="list-item-actions">
        <button class="btn-icon" onclick="editTradeOffer(${idx})" title="Edit">✏️</button>
        <button class="btn-icon" onclick="deleteTradeOffer(${idx})" title="Delete">🗑️</button>
      </div>
    </div>`;
  }).join('');
}
...
function editTradeOffer(idx) {
  ...
  document.getElementById('toDescription').value= r[1] || '';
  document.getElementById('toQuantity').value   = r[2] || '1';
  document.getElementById('toNotes').value      = r[3] || '';
  document.getElementById('toTags').value       = r[4] || '';
  document.getElementById('addTradeOfferModal').style.display = 'block';
}
...
async function saveTradeOffer(e) {
  ...
  const row = [
    ...
    document.getElementById('toDescription').value.trim(),
    document.getElementById('toQuantity').value || '1',
    document.getElementById('toNotes').value.trim(),
    document.getElementById('toTags').value.trim(),
  ];
